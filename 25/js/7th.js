...
let studentNames7th = []; // Will be populated after loading
...
let class7thBirthdays = []; // Store only 7th class birthdays
...
function loadStudentData() {
    return new Promise((resolve, reject) => {
        ...
        if (window.STUDENT_DATA && window.STUDENT_DATA.STUDENT_DATA) {
            ...
            // Extract 7th class student names
            const classKeys = Object.keys(studentData);
            console.log('Available class keys:', classKeys);
            
            // Find 7th class key (handles various formats)
            const twelfthClassKey = classKeys.find(key => {
                const normalizedKey = key.toLowerCase().replace(/[^a-z0-9]/g, '');
                return normalizedKey === '7th' || normalizedKey === '7th' || normalizedKey.includes('7th');
            });
            
            if (twelfthClassKey) {
                console.log(`Found 7th class with key: "${twelfthClassKey}"`);
                studentNames7th = studentData[twelfthClassKey]
                    .filter(student => student && student.name)
                    .map(student => student.name.trim());
                console.log(`Loaded ${studentNames7th.length} students for Class 7th:`, studentNames7th);
            } else {
                console.error('7th class data not found in STUDENT_DATA');
                // Try any key that might be 7th
                for (const key of classKeys) {
                    if (key.toLowerCase().includes('7th')) {
                        studentNames7th = studentData[key]
                            .filter(student => student && student.name)
                            .map(student => student.name.trim());
                        console.log(`Loaded ${studentNames7th.length} students using key: "${key}"`);
                        break;
                    }
                }
            }
            
            if (studentNames7th.length === 0) {
                console.warn('No student names found for Class 7th');
            }
            ...
            fetch('json/class.json')
                ...
                .then(data => {
                    ...
                    if (twelfthClassKey) {
                        ...
                        studentNames7th = studentData[twelfthClassKey]
                            .filter(student => student && student.name)
                            .map(student => student.name.trim());
                        console.log(`Loaded ${studentNames7th.length} students for Class 7th`);
                    } else {
                        console.error('7th class data not found in STUDENT_DATA');
                        // Fallback
                        const fallbackKey = classKeys.find(key => key.toLowerCase().includes('7th'));
                        if (fallbackKey) {
                            studentNames7th = studentData[fallbackKey]
                                .filter(student => student && student.name)
                                .map(student => student.name.trim());
                            console.log(`Loaded ${studentNames7th.length} students using fallback key: "${fallbackKey}"`);
                        }
                    }
                ...
                .catch(error => {
                    ...
                    studentNames7th = []; // Empty array as fallback
                    reject(error);
                });
        }
    });
...
// Helper function to check if class is 7th
function isClass7th(className) {
    if (!className) return false;
    const normalized = normalizeClassName(className);
    return normalized === '7th' || normalized === '7th' || normalized.includes('7th');
}
...
async function loadBirthdays() {
    try {
        ...
        // Filter for 7th class only using the helper function
        class7thBirthdays = data.filter(row => {
            if (!row || !row.class) return false;
            return isClass7th(row.class);
        });
        
        console.log(`Class 7th birthdays found: ${class7thBirthdays.length}`);
        ...
        if (todayList.length > 0) {
            ...
            todayList.forEach(birthday => {
                ...
                html += `
                    <div class="birthday-card class-7th">
                        ...
                        <div class="student-class">Class 7th</div>
...
function showCurrentMonthBirthdays() {
    ...
    const monthBirthdays = class7thBirthdays.filter(row => {
    ...
    if (monthBirthdays.length > 0) {
        monthBirthdays.forEach(birthday => {
            ...
            birthdaysHTML += `
                <div class="student-item class-7th">
...
// Filter notices by month AND Class (7th ya All)
function filterNotices(notices, month) {
    ...
    // Then filter by Class column (show only if Class is 'All' or 7th)
    filtered = filtered.filter(notice => {
        const noticeClass = notice.Class ? notice.Class.toString().trim() : '';
        return noticeClass.toLowerCase() === 'all' || isClass7th(noticeClass);
    });
...
function updateStats(notices) {
    ...
    notices.forEach(notice => {
        ...
        // Count notices for Class 7th
        const noticeClass = notice.Class ? notice.Class.toString().trim() : '';
        if (noticeClass.toLowerCase() === 'all' || isClass7th(noticeClass)) {
            class7thCount++;
        }
    });
...
async function fetchNoticeData() {
    try {
        ...
        const class7thNotices = data.filter(row => {
            ...
            return isClass7th(rowClass) && rowDateFormatted === todayFormatted && row.Notices;
        });
        
        if (class7thNotices.length > 0) {
            const latestNotice = class7thNotices[0];
    ...
    try {
        ...
        if (todaysHouseAssembly) {
            ...
            if (todaysClassAssembly) {
                ...
                if (isClass7th(todayAssembly.className)) {
                    applyThemeBasedOnHouse('7th');
                    showAssemblyBanner('Class 7th');
...
function showAssemblyBanner(houseOrClassName) {
    ...
    const bannerColors = {
        ...
        '7th': 'linear-gradient(135deg, #1565c0, #1976d2, #2196f3)'
    };
...
async function checkActivityData() {
    try {
        ...
        const class7thActivities = data.filter(row => 
            isClass7th(row.Class) || isClass7th(row['Select Class'])
        );
        
        if (class7thActivities.length === 0) {
...
async function checkStudyMaterialData() {
    ...
    for (const material of studyMaterials) {
        try {
            ...
            let filteredData = [];
            filteredData = data.filter(row => isClass7th(row.Class));
...
async function checkPPTData() {
    try {
        ...
        const class7thPPT = data.filter(row => isClass7th(row.Class));
        
        if (class7thPPT.length === 0) {
...
async function fetchTeacherData() {
    try {
        ...
        const class7thTeacher = data.find(row => isClass7th(row.Class));
        if (class7thTeacher) {
            teacherData = {
                name: class7thTeacher["Teacher's name"],
                dob: class7thTeacher.dob
            };
...
async function fetchTestData() {
    try {
        const [attendanceRes, marksRes] = await Promise.all([
            fetch(`${API_BASE}/${SHEET_ID}/7th_Test_Attend`),
            fetch(`${API_BASE}/${SHEET_ID}/7th_Test_Marks`)
        ]);
...
async function fetchResourcesData() {
    try {
        ...
        const data = await response.json();
        resourcesData = data.filter(row => isClass7th(row['Select Class']));
    } catch (error) {
...
async function fetchMathsMarksData() {
    try {
        ...
        const data = await response.json();
        mathsMarksData = data.filter(row => isClass7th(row.Class));
    } catch (error) {
...
async function fetchNotebookData() {
    try {
        ...
        const data = await response.json();
        notebookData = data.filter(row => isClass7th(row.Class));
    } catch (error) {
...
// PPT Materials
function showPPT() {
    popupTitle.textContent = "PPT Materials - Class 7th";
        ...
        if (class7thPPT.length === 0) {
            document.getElementById('pptContainer-materials').innerHTML = `
                <div style="text-align: center; padding: 40px; color: var(--muted);">
                    ...
                    <p>No lecture materials found for Class 7th.</p>
                </div>
            `;
            return;
        }
        
        displayPPTMaterials(class7thPPT);
    } catch (error) {
...
function showAllStudentsMarks() {
    ...
    testMarksData.forEach(test => {
        ...
        studentNames7th.forEach(studentName => {
    ...
    const sortedStudents = studentNames7th.sort((a, b) => a.localeCompare(b));
...
function downloadStudentMarksReport(studentName) {
    ...
    doc.setFontSize(12);
    doc.text("Class 7th - Academic Year 2025-26", 105, 40, { align: "center" });
    ...
    doc.text("Generated by Class 7th Dashboard", 105, 280, { align: "center" });
    ...
    html += `
        ...
        <div class="test-summary">
            <div class="summary-card">
                ...
                <div class="summary-value">${studentNames7th.length}</div>
            </div>
            <div class="summary-card">
...
async function loadQuickActions() {
    try {
        ...
        const class7thActions = quickActionsData.filter(row => 
            isClass7th(row.Class)
        );
        
        if (class7thActions.length === 0) {
            container.innerHTML = '<p style="text-align: center; grid-column: 1 / -1; color: #666;">No quick actions available for Class 7th</p>';
            return;
        }
        ...
        class7thActions.forEach(resource => {
...
function loadSMPSData() {
    fetch(SMPS_SHEET_URL)
        ...
        .then(data => {
            ...
            const class7thData = studentData.filter(row => {
            ...
            if (class7thData.length === 0) {
                document.getElementById('smpsPrintArea').innerHTML = `
                    <div style="text-align: center; padding: 40px; color: var(--light-muted);">
                        ...
                        <p>No SMPS data found for Class 7th with 'submit' status.</p>
                    </div>
                `;
                return;
            }

            smpsOriginalData = [smpsRawData[0], subHeaderRow, ...class7thData];
...
function downloadSMPSExcel() {
    ...
    link.setAttribute("download", `SMPS_Class7th_Data_${new Date().toISOString().split('T')[0]}.csv`);
...
async function loadActivityData() {
    try {
        ...
        displayActivityData(class7thActivities);
    } catch (error) {
...
function displayActivityData(activities) {
    ...
    if (activities.length === 0) {
        activityContent.innerHTML = '<p>No activities available for Class 7th.</p>';
        return;
    }
