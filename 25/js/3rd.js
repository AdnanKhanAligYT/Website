...
let studentNames3rd = []; // Will be populated after loading
...
let class3rdBirthdays = []; // Store only 3rd class birthdays
...
function loadStudentData() {
    return new Promise((resolve, reject) => {
        ...
        if (window.STUDENT_DATA && window.STUDENT_DATA.STUDENT_DATA) {
            ...
            // Extract 3rd class student names
            const classKeys = Object.keys(studentData);
            console.log('Available class keys:', classKeys);
            
            // Find 3rd class key (handles various formats)
            const twelfthClassKey = classKeys.find(key => {
                const normalizedKey = key.toLowerCase().replace(/[^a-z0-9]/g, '');
                return normalizedKey === '3rd' || normalizedKey === '3rd' || normalizedKey.includes('3rd');
            });
            
            if (twelfthClassKey) {
                console.log(`Found 3rd class with key: "${twelfthClassKey}"`);
                studentNames3rd = studentData[twelfthClassKey]
                    .filter(student => student && student.name)
                    .map(student => student.name.trim());
                console.log(`Loaded ${studentNames3rd.length} students for Class 3rd:`, studentNames3rd);
            } else {
                console.error('3rd class data not found in STUDENT_DATA');
                // Try any key that might be 3rd
                for (const key of classKeys) {
                    if (key.toLowerCase().includes('3rd')) {
                        studentNames3rd = studentData[key]
                            .filter(student => student && student.name)
                            .map(student => student.name.trim());
                        console.log(`Loaded ${studentNames3rd.length} students using key: "${key}"`);
                        break;
                    }
                }
            }
            
            if (studentNames3rd.length === 0) {
                console.warn('No student names found for Class 3rd');
            }
            ...
            fetch('json/class.json')
                ...
                .then(data => {
                    ...
                    if (twelfthClassKey) {
                        ...
                        studentNames3rd = studentData[twelfthClassKey]
                            .filter(student => student && student.name)
                            .map(student => student.name.trim());
                        console.log(`Loaded ${studentNames3rd.length} students for Class 3rd`);
                    } else {
                        console.error('3rd class data not found in STUDENT_DATA');
                        // Fallback
                        const fallbackKey = classKeys.find(key => key.toLowerCase().includes('3rd'));
                        if (fallbackKey) {
                            studentNames3rd = studentData[fallbackKey]
                                .filter(student => student && student.name)
                                .map(student => student.name.trim());
                            console.log(`Loaded ${studentNames3rd.length} students using fallback key: "${fallbackKey}"`);
                        }
                    }
                ...
                .catch(error => {
                    ...
                    studentNames3rd = []; // Empty array as fallback
                    reject(error);
                });
        }
    });
...
// Helper function to check if class is 3rd
function isClass3rd(className) {
    if (!className) return false;
    const normalized = normalizeClassName(className);
    return normalized === '3rd' || normalized === '3rd' || normalized.includes('3rd');
}
...
async function loadBirthdays() {
    try {
        ...
        // Filter for 3rd class only using the helper function
        class3rdBirthdays = data.filter(row => {
            if (!row || !row.class) return false;
            return isClass3rd(row.class);
        });
        
        console.log(`Class 3rd birthdays found: ${class3rdBirthdays.length}`);
        ...
        if (todayList.length > 0) {
            ...
            todayList.forEach(birthday => {
                ...
                html += `
                    <div class="birthday-card class-3rd">
                        ...
                        <div class="student-class">Class 3rd</div>
...
function showCurrentMonthBirthdays() {
    ...
    const monthBirthdays = class3rdBirthdays.filter(row => {
    ...
    if (monthBirthdays.length > 0) {
        monthBirthdays.forEach(birthday => {
            ...
            birthdaysHTML += `
                <div class="student-item class-3rd">
...
// Filter notices by month AND Class (3rd ya All)
function filterNotices(notices, month) {
    ...
    // Then filter by Class column (show only if Class is 'All' or 3rd)
    filtered = filtered.filter(notice => {
        const noticeClass = notice.Class ? notice.Class.toString().trim() : '';
        return noticeClass.toLowerCase() === 'all' || isClass3rd(noticeClass);
    });
...
function updateStats(notices) {
    ...
    notices.forEach(notice => {
        ...
        // Count notices for Class 3rd
        const noticeClass = notice.Class ? notice.Class.toString().trim() : '';
        if (noticeClass.toLowerCase() === 'all' || isClass3rd(noticeClass)) {
            class3rdCount++;
        }
    });
...
async function fetchNoticeData() {
    try {
        ...
        const class3rdNotices = data.filter(row => {
            ...
            return isClass3rd(rowClass) && rowDateFormatted === todayFormatted && row.Notices;
        });
        
        if (class3rdNotices.length > 0) {
            const latestNotice = class3rdNotices[0];
    ...
    try {
        ...
        if (todaysHouseAssembly) {
            ...
            if (todaysClassAssembly) {
                ...
                if (isClass3rd(todayAssembly.className)) {
                    applyThemeBasedOnHouse('3rd');
                    showAssemblyBanner('Class 3rd');
...
function showAssemblyBanner(houseOrClassName) {
    ...
    const bannerColors = {
        ...
        '3rd': 'linear-gradient(135deg, #1565c0, #1976d2, #2196f3)'
    };
...
async function checkActivityData() {
    try {
        ...
        const class3rdActivities = data.filter(row => 
            isClass3rd(row.Class) || isClass3rd(row['Select Class'])
        );
        
        if (class3rdActivities.length === 0) {
...
async function checkStudyMaterialData() {
    ...
    for (const material of studyMaterials) {
        try {
            ...
            let filteredData = [];
            filteredData = data.filter(row => isClass3rd(row.Class));
...
async function checkPPTData() {
    try {
        ...
        const class3rdPPT = data.filter(row => isClass3rd(row.Class));
        
        if (class3rdPPT.length === 0) {
...
async function fetchTeacherData() {
    try {
        ...
        const class3rdTeacher = data.find(row => isClass3rd(row.Class));
        if (class3rdTeacher) {
            teacherData = {
                name: class3rdTeacher["Teacher's name"],
                dob: class3rdTeacher.dob
            };
...
async function fetchTestData() {
    try {
        const [attendanceRes, marksRes] = await Promise.all([
            fetch(`${API_BASE}/${SHEET_ID}/3rd_Test_Attend`),
            fetch(`${API_BASE}/${SHEET_ID}/3rd_Test_Marks`)
        ]);
...
async function fetchResourcesData() {
    try {
        ...
        const data = await response.json();
        resourcesData = data.filter(row => isClass3rd(row['Select Class']));
    } catch (error) {
...
async function fetchMathsMarksData() {
    try {
        ...
        const data = await response.json();
        mathsMarksData = data.filter(row => isClass3rd(row.Class));
    } catch (error) {
...
async function fetchNotebookData() {
    try {
        ...
        const data = await response.json();
        notebookData = data.filter(row => isClass3rd(row.Class));
    } catch (error) {
...
// PPT Materials
function showPPT() {
    popupTitle.textContent = "PPT Materials - Class 3rd";
        ...
        if (class3rdPPT.length === 0) {
            document.getElementById('pptContainer-materials').innerHTML = `
                <div style="text-align: center; padding: 40px; color: var(--muted);">
                    ...
                    <p>No lecture materials found for Class 3rd.</p>
                </div>
            `;
            return;
        }
        
        displayPPTMaterials(class3rdPPT);
    } catch (error) {
...
function showAllStudentsMarks() {
    ...
    testMarksData.forEach(test => {
        ...
        studentNames3rd.forEach(studentName => {
    ...
    const sortedStudents = studentNames3rd.sort((a, b) => a.localeCompare(b));
...
function downloadStudentMarksReport(studentName) {
    ...
    doc.setFontSize(12);
    doc.text("Class 3rd - Academic Year 2025-26", 105, 40, { align: "center" });
    ...
    doc.text("Generated by Class 3rd Dashboard", 105, 280, { align: "center" });
    ...
    html += `
        ...
        <div class="test-summary">
            <div class="summary-card">
                ...
                <div class="summary-value">${studentNames3rd.length}</div>
            </div>
            <div class="summary-card">
...
async function loadQuickActions() {
    try {
        ...
        const class3rdActions = quickActionsData.filter(row => 
            isClass3rd(row.Class)
        );
        
        if (class3rdActions.length === 0) {
            container.innerHTML = '<p style="text-align: center; grid-column: 1 / -1; color: #666;">No quick actions available for Class 3rd</p>';
            return;
        }
        ...
        class3rdActions.forEach(resource => {
...
function loadSMPSData() {
    fetch(SMPS_SHEET_URL)
        ...
        .then(data => {
            ...
            const class3rdData = studentData.filter(row => {
            ...
            if (class3rdData.length === 0) {
                document.getElementById('smpsPrintArea').innerHTML = `
                    <div style="text-align: center; padding: 40px; color: var(--light-muted);">
                        ...
                        <p>No SMPS data found for Class 3rd with 'submit' status.</p>
                    </div>
                `;
                return;
            }

            smpsOriginalData = [smpsRawData[0], subHeaderRow, ...class3rdData];
...
function downloadSMPSExcel() {
    ...
    link.setAttribute("download", `SMPS_Class3rd_Data_${new Date().toISOString().split('T')[0]}.csv`);
...
async function loadActivityData() {
    try {
        ...
        displayActivityData(class3rdActivities);
    } catch (error) {
...
function displayActivityData(activities) {
    ...
    if (activities.length === 0) {
        activityContent.innerHTML = '<p>No activities available for Class 3rd.</p>';
        return;
    }
