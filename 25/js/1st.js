...
let studentNames1st = []; // Will be populated after loading
...
let class1stBirthdays = []; // Store only 1st class birthdays
...
function loadStudentData() {
    return new Promise((resolve, reject) => {
        ...
        if (window.STUDENT_DATA && window.STUDENT_DATA.STUDENT_DATA) {
            ...
            // Extract 1st class student names
            const classKeys = Object.keys(studentData);
            console.log('Available class keys:', classKeys);
            
            // Find 1st class key (handles various formats)
            const twelfthClassKey = classKeys.find(key => {
                const normalizedKey = key.toLowerCase().replace(/[^a-z0-9]/g, '');
                return normalizedKey === '1st' || normalizedKey === '1st' || normalizedKey.includes('1st');
            });
            
            if (twelfthClassKey) {
                console.log(`Found 1st class with key: "${twelfthClassKey}"`);
                studentNames1st = studentData[twelfthClassKey]
                    .filter(student => student && student.name)
                    .map(student => student.name.trim());
                console.log(`Loaded ${studentNames1st.length} students for Class 1st:`, studentNames1st);
            } else {
                console.error('1st class data not found in STUDENT_DATA');
                // Try any key that might be 1st
                for (const key of classKeys) {
                    if (key.toLowerCase().includes('1st')) {
                        studentNames1st = studentData[key]
                            .filter(student => student && student.name)
                            .map(student => student.name.trim());
                        console.log(`Loaded ${studentNames1st.length} students using key: "${key}"`);
                        break;
                    }
                }
            }
            
            if (studentNames1st.length === 0) {
                console.warn('No student names found for Class 1st');
            }
            ...
            fetch('json/class.json')
                ...
                .then(data => {
                    ...
                    if (twelfthClassKey) {
                        ...
                        studentNames1st = studentData[twelfthClassKey]
                            .filter(student => student && student.name)
                            .map(student => student.name.trim());
                        console.log(`Loaded ${studentNames1st.length} students for Class 1st`);
                    } else {
                        console.error('1st class data not found in STUDENT_DATA');
                        // Fallback
                        const fallbackKey = classKeys.find(key => key.toLowerCase().includes('1st'));
                        if (fallbackKey) {
                            studentNames1st = studentData[fallbackKey]
                                .filter(student => student && student.name)
                                .map(student => student.name.trim());
                            console.log(`Loaded ${studentNames1st.length} students using fallback key: "${fallbackKey}"`);
                        }
                    }
                ...
                .catch(error => {
                    ...
                    studentNames1st = []; // Empty array as fallback
                    reject(error);
                });
        }
    });
...
// Helper function to check if class is 1st
function isClass1st(className) {
    if (!className) return false;
    const normalized = normalizeClassName(className);
    return normalized === '1st' || normalized === '1st' || normalized.includes('1st');
}
...
async function loadBirthdays() {
    try {
        ...
        // Filter for 1st class only using the helper function
        class1stBirthdays = data.filter(row => {
            if (!row || !row.class) return false;
            return isClass1st(row.class);
        });
        
        console.log(`Class 1st birthdays found: ${class1stBirthdays.length}`);
        ...
        if (todayList.length > 0) {
            ...
            todayList.forEach(birthday => {
                ...
                html += `
                    <div class="birthday-card class-1st">
                        ...
                        <div class="student-class">Class 1st</div>
...
function showCurrentMonthBirthdays() {
    ...
    const monthBirthdays = class1stBirthdays.filter(row => {
    ...
    if (monthBirthdays.length > 0) {
        monthBirthdays.forEach(birthday => {
            ...
            birthdaysHTML += `
                <div class="student-item class-1st">
...
// Filter notices by month AND Class (1st ya All)
function filterNotices(notices, month) {
    ...
    // Then filter by Class column (show only if Class is 'All' or 1st)
    filtered = filtered.filter(notice => {
        const noticeClass = notice.Class ? notice.Class.toString().trim() : '';
        return noticeClass.toLowerCase() === 'all' || isClass1st(noticeClass);
    });
...
function updateStats(notices) {
    ...
    notices.forEach(notice => {
        ...
        // Count notices for Class 1st
        const noticeClass = notice.Class ? notice.Class.toString().trim() : '';
        if (noticeClass.toLowerCase() === 'all' || isClass1st(noticeClass)) {
            class1stCount++;
        }
    });
...
async function fetchNoticeData() {
    try {
        ...
        const class1stNotices = data.filter(row => {
            ...
            return isClass1st(rowClass) && rowDateFormatted === todayFormatted && row.Notices;
        });
        
        if (class1stNotices.length > 0) {
            const latestNotice = class1stNotices[0];
    ...
    try {
        ...
        if (todaysHouseAssembly) {
            ...
            if (todaysClassAssembly) {
                ...
                if (isClass1st(todayAssembly.className)) {
                    applyThemeBasedOnHouse('1st');
                    showAssemblyBanner('Class 1st');
...
function showAssemblyBanner(houseOrClassName) {
    ...
    const bannerColors = {
        ...
        '1st': 'linear-gradient(135deg, #1565c0, #1976d2, #2196f3)'
    };
...
async function checkActivityData() {
    try {
        ...
        const class1stActivities = data.filter(row => 
            isClass1st(row.Class) || isClass1st(row['Select Class'])
        );
        
        if (class1stActivities.length === 0) {
...
async function checkStudyMaterialData() {
    ...
    for (const material of studyMaterials) {
        try {
            ...
            let filteredData = [];
            filteredData = data.filter(row => isClass1st(row.Class));
...
async function checkPPTData() {
    try {
        ...
        const class1stPPT = data.filter(row => isClass1st(row.Class));
        
        if (class1stPPT.length === 0) {
...
async function fetchTeacherData() {
    try {
        ...
        const class1stTeacher = data.find(row => isClass1st(row.Class));
        if (class1stTeacher) {
            teacherData = {
                name: class1stTeacher["Teacher's name"],
                dob: class1stTeacher.dob
            };
...
async function fetchTestData() {
    try {
        const [attendanceRes, marksRes] = await Promise.all([
            fetch(`${API_BASE}/${SHEET_ID}/1st_Test_Attend`),
            fetch(`${API_BASE}/${SHEET_ID}/1st_Test_Marks`)
        ]);
...
async function fetchResourcesData() {
    try {
        ...
        const data = await response.json();
        resourcesData = data.filter(row => isClass1st(row['Select Class']));
    } catch (error) {
...
async function fetchMathsMarksData() {
    try {
        ...
        const data = await response.json();
        mathsMarksData = data.filter(row => isClass1st(row.Class));
    } catch (error) {
...
async function fetchNotebookData() {
    try {
        ...
        const data = await response.json();
        notebookData = data.filter(row => isClass1st(row.Class));
    } catch (error) {
...
// PPT Materials
function showPPT() {
    popupTitle.textContent = "PPT Materials - Class 1st";
        ...
        if (class1stPPT.length === 0) {
            document.getElementById('pptContainer-materials').innerHTML = `
                <div style="text-align: center; padding: 40px; color: var(--muted);">
                    ...
                    <p>No lecture materials found for Class 1st.</p>
                </div>
            `;
            return;
        }
        
        displayPPTMaterials(class1stPPT);
    } catch (error) {
...
function showAllStudentsMarks() {
    ...
    testMarksData.forEach(test => {
        ...
        studentNames1st.forEach(studentName => {
    ...
    const sortedStudents = studentNames1st.sort((a, b) => a.localeCompare(b));
...
function downloadStudentMarksReport(studentName) {
    ...
    doc.setFontSize(12);
    doc.text("Class 1st - Academic Year 2025-26", 105, 40, { align: "center" });
    ...
    doc.text("Generated by Class 1st Dashboard", 105, 280, { align: "center" });
    ...
    html += `
        ...
        <div class="test-summary">
            <div class="summary-card">
                ...
                <div class="summary-value">${studentNames1st.length}</div>
            </div>
            <div class="summary-card">
...
async function loadQuickActions() {
    try {
        ...
        const class1stActions = quickActionsData.filter(row => 
            isClass1st(row.Class)
        );
        
        if (class1stActions.length === 0) {
            container.innerHTML = '<p style="text-align: center; grid-column: 1 / -1; color: #666;">No quick actions available for Class 1st</p>';
            return;
        }
        ...
        class1stActions.forEach(resource => {
...
function loadSMPSData() {
    fetch(SMPS_SHEET_URL)
        ...
        .then(data => {
            ...
            const class1stData = studentData.filter(row => {
            ...
            if (class1stData.length === 0) {
                document.getElementById('smpsPrintArea').innerHTML = `
                    <div style="text-align: center; padding: 40px; color: var(--light-muted);">
                        ...
                        <p>No SMPS data found for Class 1st with 'submit' status.</p>
                    </div>
                `;
                return;
            }

            smpsOriginalData = [smpsRawData[0], subHeaderRow, ...class1stData];
...
function downloadSMPSExcel() {
    ...
    link.setAttribute("download", `SMPS_Class1st_Data_${new Date().toISOString().split('T')[0]}.csv`);
...
async function loadActivityData() {
    try {
        ...
        displayActivityData(class1stActivities);
    } catch (error) {
...
function displayActivityData(activities) {
    ...
    if (activities.length === 0) {
        activityContent.innerHTML = '<p>No activities available for Class 1st.</p>';
        return;
    }
