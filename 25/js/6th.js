...
let studentNames6th = []; // Will be populated after loading
...
let class6thBirthdays = []; // Store only 6th class birthdays
...
function loadStudentData() {
    return new Promise((resolve, reject) => {
        ...
        if (window.STUDENT_DATA && window.STUDENT_DATA.STUDENT_DATA) {
            ...
            // Extract 6th class student names
            const classKeys = Object.keys(studentData);
            console.log('Available class keys:', classKeys);
            
            // Find 6th class key (handles various formats)
            const twelfthClassKey = classKeys.find(key => {
                const normalizedKey = key.toLowerCase().replace(/[^a-z0-9]/g, '');
                return normalizedKey === '6th' || normalizedKey === '6th' || normalizedKey.includes('6th');
            });
            
            if (twelfthClassKey) {
                console.log(`Found 6th class with key: "${twelfthClassKey}"`);
                studentNames6th = studentData[twelfthClassKey]
                    .filter(student => student && student.name)
                    .map(student => student.name.trim());
                console.log(`Loaded ${studentNames6th.length} students for Class 6th:`, studentNames6th);
            } else {
                console.error('6th class data not found in STUDENT_DATA');
                // Try any key that might be 6th
                for (const key of classKeys) {
                    if (key.toLowerCase().includes('6th')) {
                        studentNames6th = studentData[key]
                            .filter(student => student && student.name)
                            .map(student => student.name.trim());
                        console.log(`Loaded ${studentNames6th.length} students using key: "${key}"`);
                        break;
                    }
                }
            }
            
            if (studentNames6th.length === 0) {
                console.warn('No student names found for Class 6th');
            }
            ...
            fetch('json/class.json')
                ...
                .then(data => {
                    ...
                    if (twelfthClassKey) {
                        ...
                        studentNames6th = studentData[twelfthClassKey]
                            .filter(student => student && student.name)
                            .map(student => student.name.trim());
                        console.log(`Loaded ${studentNames6th.length} students for Class 6th`);
                    } else {
                        console.error('6th class data not found in STUDENT_DATA');
                        // Fallback
                        const fallbackKey = classKeys.find(key => key.toLowerCase().includes('6th'));
                        if (fallbackKey) {
                            studentNames6th = studentData[fallbackKey]
                                .filter(student => student && student.name)
                                .map(student => student.name.trim());
                            console.log(`Loaded ${studentNames6th.length} students using fallback key: "${fallbackKey}"`);
                        }
                    }
                ...
                .catch(error => {
                    ...
                    studentNames6th = []; // Empty array as fallback
                    reject(error);
                });
        }
    });
...
// Helper function to check if class is 6th
function isClass6th(className) {
    if (!className) return false;
    const normalized = normalizeClassName(className);
    return normalized === '6th' || normalized === '6th' || normalized.includes('6th');
}
...
async function loadBirthdays() {
    try {
        ...
        // Filter for 6th class only using the helper function
        class6thBirthdays = data.filter(row => {
            if (!row || !row.class) return false;
            return isClass6th(row.class);
        });
        
        console.log(`Class 6th birthdays found: ${class6thBirthdays.length}`);
        ...
        if (todayList.length > 0) {
            ...
            todayList.forEach(birthday => {
                ...
                html += `
                    <div class="birthday-card class-6th">
                        ...
                        <div class="student-class">Class 6th</div>
...
function showCurrentMonthBirthdays() {
    ...
    const monthBirthdays = class6thBirthdays.filter(row => {
    ...
    if (monthBirthdays.length > 0) {
        monthBirthdays.forEach(birthday => {
            ...
            birthdaysHTML += `
                <div class="student-item class-6th">
...
// Filter notices by month AND Class (6th ya All)
function filterNotices(notices, month) {
    ...
    // Then filter by Class column (show only if Class is 'All' or 6th)
    filtered = filtered.filter(notice => {
        const noticeClass = notice.Class ? notice.Class.toString().trim() : '';
        return noticeClass.toLowerCase() === 'all' || isClass6th(noticeClass);
    });
...
function updateStats(notices) {
    ...
    notices.forEach(notice => {
        ...
        // Count notices for Class 6th
        const noticeClass = notice.Class ? notice.Class.toString().trim() : '';
        if (noticeClass.toLowerCase() === 'all' || isClass6th(noticeClass)) {
            class6thCount++;
        }
    });
...
async function fetchNoticeData() {
    try {
        ...
        const class6thNotices = data.filter(row => {
            ...
            return isClass6th(rowClass) && rowDateFormatted === todayFormatted && row.Notices;
        });
        
        if (class6thNotices.length > 0) {
            const latestNotice = class6thNotices[0];
    ...
    try {
        ...
        if (todaysHouseAssembly) {
            ...
            if (todaysClassAssembly) {
                ...
                if (isClass6th(todayAssembly.className)) {
                    applyThemeBasedOnHouse('6th');
                    showAssemblyBanner('Class 6th');
...
function showAssemblyBanner(houseOrClassName) {
    ...
    const bannerColors = {
        ...
        '6th': 'linear-gradient(135deg, #1565c0, #1976d2, #2196f3)'
    };
...
async function checkActivityData() {
    try {
        ...
        const class6thActivities = data.filter(row => 
            isClass6th(row.Class) || isClass6th(row['Select Class'])
        );
        
        if (class6thActivities.length === 0) {
...
async function checkStudyMaterialData() {
    ...
    for (const material of studyMaterials) {
        try {
            ...
            let filteredData = [];
            filteredData = data.filter(row => isClass6th(row.Class));
...
async function checkPPTData() {
    try {
        ...
        const class6thPPT = data.filter(row => isClass6th(row.Class));
        
        if (class6thPPT.length === 0) {
...
async function fetchTeacherData() {
    try {
        ...
        const class6thTeacher = data.find(row => isClass6th(row.Class));
        if (class6thTeacher) {
            teacherData = {
                name: class6thTeacher["Teacher's name"],
                dob: class6thTeacher.dob
            };
...
async function fetchTestData() {
    try {
        const [attendanceRes, marksRes] = await Promise.all([
            fetch(`${API_BASE}/${SHEET_ID}/6th_Test_Attend`),
            fetch(`${API_BASE}/${SHEET_ID}/6th_Test_Marks`)
        ]);
...
async function fetchResourcesData() {
    try {
        ...
        const data = await response.json();
        resourcesData = data.filter(row => isClass6th(row['Select Class']));
    } catch (error) {
...
async function fetchMathsMarksData() {
    try {
        ...
        const data = await response.json();
        mathsMarksData = data.filter(row => isClass6th(row.Class));
    } catch (error) {
...
async function fetchNotebookData() {
    try {
        ...
        const data = await response.json();
        notebookData = data.filter(row => isClass6th(row.Class));
    } catch (error) {
...
// PPT Materials
function showPPT() {
    popupTitle.textContent = "PPT Materials - Class 6th";
        ...
        if (class6thPPT.length === 0) {
            document.getElementById('pptContainer-materials').innerHTML = `
                <div style="text-align: center; padding: 40px; color: var(--muted);">
                    ...
                    <p>No lecture materials found for Class 6th.</p>
                </div>
            `;
            return;
        }
        
        displayPPTMaterials(class6thPPT);
    } catch (error) {
...
function showAllStudentsMarks() {
    ...
    testMarksData.forEach(test => {
        ...
        studentNames6th.forEach(studentName => {
    ...
    const sortedStudents = studentNames6th.sort((a, b) => a.localeCompare(b));
...
function downloadStudentMarksReport(studentName) {
    ...
    doc.setFontSize(12);
    doc.text("Class 6th - Academic Year 2025-26", 105, 40, { align: "center" });
    ...
    doc.text("Generated by Class 6th Dashboard", 105, 280, { align: "center" });
    ...
    html += `
        ...
        <div class="test-summary">
            <div class="summary-card">
                ...
                <div class="summary-value">${studentNames6th.length}</div>
            </div>
            <div class="summary-card">
...
async function loadQuickActions() {
    try {
        ...
        const class6thActions = quickActionsData.filter(row => 
            isClass6th(row.Class)
        );
        
        if (class6thActions.length === 0) {
            container.innerHTML = '<p style="text-align: center; grid-column: 1 / -1; color: #666;">No quick actions available for Class 6th</p>';
            return;
        }
        ...
        class6thActions.forEach(resource => {
...
function loadSMPSData() {
    fetch(SMPS_SHEET_URL)
        ...
        .then(data => {
            ...
            const class6thData = studentData.filter(row => {
            ...
            if (class6thData.length === 0) {
                document.getElementById('smpsPrintArea').innerHTML = `
                    <div style="text-align: center; padding: 40px; color: var(--light-muted);">
                        ...
                        <p>No SMPS data found for Class 6th with 'submit' status.</p>
                    </div>
                `;
                return;
            }

            smpsOriginalData = [smpsRawData[0], subHeaderRow, ...class6thData];
...
function downloadSMPSExcel() {
    ...
    link.setAttribute("download", `SMPS_Class6th_Data_${new Date().toISOString().split('T')[0]}.csv`);
...
async function loadActivityData() {
    try {
        ...
        displayActivityData(class6thActivities);
    } catch (error) {
...
function displayActivityData(activities) {
    ...
    if (activities.length === 0) {
        activityContent.innerHTML = '<p>No activities available for Class 6th.</p>';
        return;
    }
