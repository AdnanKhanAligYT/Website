...
let studentNames4th = []; // Will be populated after loading
...
let class4thBirthdays = []; // Store only 4th class birthdays
...
function loadStudentData() {
    return new Promise((resolve, reject) => {
        ...
        if (window.STUDENT_DATA && window.STUDENT_DATA.STUDENT_DATA) {
            ...
            // Extract 4th class student names
            const classKeys = Object.keys(studentData);
            console.log('Available class keys:', classKeys);
            
            // Find 4th class key (handles various formats)
            const twelfthClassKey = classKeys.find(key => {
                const normalizedKey = key.toLowerCase().replace(/[^a-z0-9]/g, '');
                return normalizedKey === '4th' || normalizedKey === '4th' || normalizedKey.includes('4th');
            });
            
            if (twelfthClassKey) {
                console.log(`Found 4th class with key: "${twelfthClassKey}"`);
                studentNames4th = studentData[twelfthClassKey]
                    .filter(student => student && student.name)
                    .map(student => student.name.trim());
                console.log(`Loaded ${studentNames4th.length} students for Class 4th:`, studentNames4th);
            } else {
                console.error('4th class data not found in STUDENT_DATA');
                // Try any key that might be 4th
                for (const key of classKeys) {
                    if (key.toLowerCase().includes('4th')) {
                        studentNames4th = studentData[key]
                            .filter(student => student && student.name)
                            .map(student => student.name.trim());
                        console.log(`Loaded ${studentNames4th.length} students using key: "${key}"`);
                        break;
                    }
                }
            }
            
            if (studentNames4th.length === 0) {
                console.warn('No student names found for Class 4th');
            }
            ...
            fetch('json/class.json')
                ...
                .then(data => {
                    ...
                    if (twelfthClassKey) {
                        ...
                        studentNames4th = studentData[twelfthClassKey]
                            .filter(student => student && student.name)
                            .map(student => student.name.trim());
                        console.log(`Loaded ${studentNames4th.length} students for Class 4th`);
                    } else {
                        console.error('4th class data not found in STUDENT_DATA');
                        // Fallback
                        const fallbackKey = classKeys.find(key => key.toLowerCase().includes('4th'));
                        if (fallbackKey) {
                            studentNames4th = studentData[fallbackKey]
                                .filter(student => student && student.name)
                                .map(student => student.name.trim());
                            console.log(`Loaded ${studentNames4th.length} students using fallback key: "${fallbackKey}"`);
                        }
                    }
                ...
                .catch(error => {
                    ...
                    studentNames4th = []; // Empty array as fallback
                    reject(error);
                });
        }
    });
...
// Helper function to check if class is 4th
function isClass4th(className) {
    if (!className) return false;
    const normalized = normalizeClassName(className);
    return normalized === '4th' || normalized === '4th' || normalized.includes('4th');
}
...
async function loadBirthdays() {
    try {
        ...
        // Filter for 4th class only using the helper function
        class4thBirthdays = data.filter(row => {
            if (!row || !row.class) return false;
            return isClass4th(row.class);
        });
        
        console.log(`Class 4th birthdays found: ${class4thBirthdays.length}`);
        ...
        if (todayList.length > 0) {
            ...
            todayList.forEach(birthday => {
                ...
                html += `
                    <div class="birthday-card class-4th">
                        ...
                        <div class="student-class">Class 4th</div>
...
function showCurrentMonthBirthdays() {
    ...
    const monthBirthdays = class4thBirthdays.filter(row => {
    ...
    if (monthBirthdays.length > 0) {
        monthBirthdays.forEach(birthday => {
            ...
            birthdaysHTML += `
                <div class="student-item class-4th">
...
// Filter notices by month AND Class (4th ya All)
function filterNotices(notices, month) {
    ...
    // Then filter by Class column (show only if Class is 'All' or 4th)
    filtered = filtered.filter(notice => {
        const noticeClass = notice.Class ? notice.Class.toString().trim() : '';
        return noticeClass.toLowerCase() === 'all' || isClass4th(noticeClass);
    });
...
function updateStats(notices) {
    ...
    notices.forEach(notice => {
        ...
        // Count notices for Class 4th
        const noticeClass = notice.Class ? notice.Class.toString().trim() : '';
        if (noticeClass.toLowerCase() === 'all' || isClass4th(noticeClass)) {
            class4thCount++;
        }
    });
...
async function fetchNoticeData() {
    try {
        ...
        const class4thNotices = data.filter(row => {
            ...
            return isClass4th(rowClass) && rowDateFormatted === todayFormatted && row.Notices;
        });
        
        if (class4thNotices.length > 0) {
            const latestNotice = class4thNotices[0];
    ...
    try {
        ...
        if (todaysHouseAssembly) {
            ...
            if (todaysClassAssembly) {
                ...
                if (isClass4th(todayAssembly.className)) {
                    applyThemeBasedOnHouse('4th');
                    showAssemblyBanner('Class 4th');
...
function showAssemblyBanner(houseOrClassName) {
    ...
    const bannerColors = {
        ...
        '4th': 'linear-gradient(135deg, #1565c0, #1976d2, #2196f3)'
    };
...
async function checkActivityData() {
    try {
        ...
        const class4thActivities = data.filter(row => 
            isClass4th(row.Class) || isClass4th(row['Select Class'])
        );
        
        if (class4thActivities.length === 0) {
...
async function checkStudyMaterialData() {
    ...
    for (const material of studyMaterials) {
        try {
            ...
            let filteredData = [];
            filteredData = data.filter(row => isClass4th(row.Class));
...
async function checkPPTData() {
    try {
        ...
        const class4thPPT = data.filter(row => isClass4th(row.Class));
        
        if (class4thPPT.length === 0) {
...
async function fetchTeacherData() {
    try {
        ...
        const class4thTeacher = data.find(row => isClass4th(row.Class));
        if (class4thTeacher) {
            teacherData = {
                name: class4thTeacher["Teacher's name"],
                dob: class4thTeacher.dob
            };
...
async function fetchTestData() {
    try {
        const [attendanceRes, marksRes] = await Promise.all([
            fetch(`${API_BASE}/${SHEET_ID}/4th_Test_Attend`),
            fetch(`${API_BASE}/${SHEET_ID}/4th_Test_Marks`)
        ]);
...
async function fetchResourcesData() {
    try {
        ...
        const data = await response.json();
        resourcesData = data.filter(row => isClass4th(row['Select Class']));
    } catch (error) {
...
async function fetchMathsMarksData() {
    try {
        ...
        const data = await response.json();
        mathsMarksData = data.filter(row => isClass4th(row.Class));
    } catch (error) {
...
async function fetchNotebookData() {
    try {
        ...
        const data = await response.json();
        notebookData = data.filter(row => isClass4th(row.Class));
    } catch (error) {
...
// PPT Materials
function showPPT() {
    popupTitle.textContent = "PPT Materials - Class 4th";
        ...
        if (class4thPPT.length === 0) {
            document.getElementById('pptContainer-materials').innerHTML = `
                <div style="text-align: center; padding: 40px; color: var(--muted);">
                    ...
                    <p>No lecture materials found for Class 4th.</p>
                </div>
            `;
            return;
        }
        
        displayPPTMaterials(class4thPPT);
    } catch (error) {
...
function showAllStudentsMarks() {
    ...
    testMarksData.forEach(test => {
        ...
        studentNames4th.forEach(studentName => {
    ...
    const sortedStudents = studentNames4th.sort((a, b) => a.localeCompare(b));
...
function downloadStudentMarksReport(studentName) {
    ...
    doc.setFontSize(12);
    doc.text("Class 4th - Academic Year 2025-26", 105, 40, { align: "center" });
    ...
    doc.text("Generated by Class 4th Dashboard", 105, 280, { align: "center" });
    ...
    html += `
        ...
        <div class="test-summary">
            <div class="summary-card">
                ...
                <div class="summary-value">${studentNames4th.length}</div>
            </div>
            <div class="summary-card">
...
async function loadQuickActions() {
    try {
        ...
        const class4thActions = quickActionsData.filter(row => 
            isClass4th(row.Class)
        );
        
        if (class4thActions.length === 0) {
            container.innerHTML = '<p style="text-align: center; grid-column: 1 / -1; color: #666;">No quick actions available for Class 4th</p>';
            return;
        }
        ...
        class4thActions.forEach(resource => {
...
function loadSMPSData() {
    fetch(SMPS_SHEET_URL)
        ...
        .then(data => {
            ...
            const class4thData = studentData.filter(row => {
            ...
            if (class4thData.length === 0) {
                document.getElementById('smpsPrintArea').innerHTML = `
                    <div style="text-align: center; padding: 40px; color: var(--light-muted);">
                        ...
                        <p>No SMPS data found for Class 4th with 'submit' status.</p>
                    </div>
                `;
                return;
            }

            smpsOriginalData = [smpsRawData[0], subHeaderRow, ...class4thData];
...
function downloadSMPSExcel() {
    ...
    link.setAttribute("download", `SMPS_Class4th_Data_${new Date().toISOString().split('T')[0]}.csv`);
...
async function loadActivityData() {
    try {
        ...
        displayActivityData(class4thActivities);
    } catch (error) {
...
function displayActivityData(activities) {
    ...
    if (activities.length === 0) {
        activityContent.innerHTML = '<p>No activities available for Class 4th.</p>';
        return;
    }
