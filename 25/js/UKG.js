...
let studentNamesUKG = []; // Will be populated after loading
...
let classUKGBirthdays = []; // Store only UKG class birthdays
...
function loadStudentData() {
    return new Promise((resolve, reject) => {
        ...
        if (window.STUDENT_DATA && window.STUDENT_DATA.STUDENT_DATA) {
            ...
            // Extract UKG class student names
            const classKeys = Object.keys(studentData);
            console.log('Available class keys:', classKeys);
            
            // Find UKG class key (handles various formats)
            const ukgClassKey = classKeys.find(key => {
                const normalizedKey = key.toLowerCase().replace(/[^a-z0-9]/g, '');
                return normalizedKey === 'ukg' || normalizedKey.includes('ukg');
            });
            
            if (ukgClassKey) {
                console.log(`Found UKG class with key: "${ukgClassKey}"`);
                studentNamesUKG = studentData[ukgClassKey]
                    .filter(student => student && student.name)
                    .map(student => student.name.trim());
                console.log(`Loaded ${studentNamesUKG.length} students for Class UKG:`, studentNamesUKG);
            } else {
                console.error('UKG class data not found in STUDENT_DATA');
                // Try any key that might be UKG
                for (const key of classKeys) {
                    if (key.toLowerCase().includes('ukg')) {
                        studentNamesUKG = studentData[key]
                            .filter(student => student && student.name)
                            .map(student => student.name.trim());
                        console.log(`Loaded ${studentNamesUKG.length} students using key: "${key}"`);
                        break;
                    }
                }
            }
            
            if (studentNamesUKG.length === 0) {
                console.warn('No student names found for Class UKG');
            }
            ...
            fetch('json/class.json')
                ...
                .then(data => {
                    ...
                    if (ukgClassKey) {
                        ...
                        studentNamesUKG = studentData[ukgClassKey]
                            .filter(student => student && student.name)
                            .map(student => student.name.trim());
                        console.log(`Loaded ${studentNamesUKG.length} students for Class UKG`);
                    } else {
                        console.error('UKG class data not found in STUDENT_DATA');
                        // Fallback
                        const fallbackKey = classKeys.find(key => key.toLowerCase().includes('ukg'));
                        if (fallbackKey) {
                            studentNamesUKG = studentData[fallbackKey]
                                .filter(student => student && student.name)
                                .map(student => student.name.trim());
                            console.log(`Loaded ${studentNamesUKG.length} students using fallback key: "${fallbackKey}"`);
                        }
                    }
                ...
                .catch(error => {
                    ...
                    studentNamesUKG = []; // Empty array as fallback
                    reject(error);
                });
        }
    });
...
// Helper function to check if class is UKG
function isClassUKG(className) {
    if (!className) return false;
    const normalized = normalizeClassName(className);
    return normalized === 'ukg' || normalized.includes('ukg');
}
...
async function loadBirthdays() {
    try {
        ...
        // Filter for UKG class only using the helper function
        classUKGBirthdays = data.filter(row => {
            if (!row || !row.class) return false;
            return isClassUKG(row.class);
        });
        
        console.log(`Class UKG birthdays found: ${classUKGBirthdays.length}`);
        ...
        if (todayList.length > 0) {
            ...
            todayList.forEach(birthday => {
                ...
                html += `
                    <div class="birthday-card class-UKG">
                        ...
                        <div class="student-class">Class UKG</div>
...
function showCurrentMonthBirthdays() {
    ...
    const monthBirthdays = classUKGBirthdays.filter(row => {
    ...
    if (monthBirthdays.length > 0) {
        monthBirthdays.forEach(birthday => {
            ...
            birthdaysHTML += `
                <div class="student-item class-UKG">
...
// Filter notices by month AND Class (UKG ya All)
function filterNotices(notices, month) {
    ...
    // Then filter by Class column (show only if Class is 'All' or UKG)
    filtered = filtered.filter(notice => {
        const noticeClass = notice.Class ? notice.Class.toString().trim() : '';
        return noticeClass.toLowerCase() === 'all' || isClassUKG(noticeClass);
    });
...
function updateStats(notices) {
    ...
    notices.forEach(notice => {
        ...
        // Count notices for Class UKG
        const noticeClass = notice.Class ? notice.Class.toString().trim() : '';
        if (noticeClass.toLowerCase() === 'all' || isClassUKG(noticeClass)) {
            classUKGCount++;
        }
    });
...
async function fetchNoticeData() {
    try {
        ...
        const classUKGNotices = data.filter(row => {
            ...
            return isClassUKG(rowClass) && rowDateFormatted === todayFormatted && row.Notices;
        });
        
        if (classUKGNotices.length > 0) {
            const latestNotice = classUKGNotices[0];
    ...
    try {
        ...
        if (todaysHouseAssembly) {
            ...
            if (todaysClassAssembly) {
                ...
                if (isClassUKG(todayAssembly.className)) {
                    applyThemeBasedOnHouse('UKG');
                    showAssemblyBanner('Class UKG');
...
function showAssemblyBanner(houseOrClassName) {
    ...
    const bannerColors = {
        ...
        'UKG': 'linear-gradient(135deg, #1565c0, #1976d2, #2196f3)'
    };
...
async function checkActivityData() {
    try {
        ...
        const classUKGActivities = data.filter(row => 
            isClassUKG(row.Class) || isClassUKG(row['Select Class'])
        );
        
        if (classUKGActivities.length === 0) {
...
async function checkStudyMaterialData() {
    ...
    for (const material of studyMaterials) {
        try {
            ...
            let filteredData = [];
            filteredData = data.filter(row => isClassUKG(row.Class));
...
async function checkPPTData() {
    try {
        ...
        const classUKGPPT = data.filter(row => isClassUKG(row.Class));
        
        if (classUKGPPT.length === 0) {
...
async function fetchTeacherData() {
    try {
        ...
        const classUKGTeacher = data.find(row => isClassUKG(row.Class));
        if (classUKGTeacher) {
            teacherData = {
                name: classUKGTeacher["Teacher's name"],
                dob: classUKGTeacher.dob
            };
...
async function fetchTestData() {
    try {
        const [attendanceRes, marksRes] = await Promise.all([
            fetch(`${API_BASE}/${SHEET_ID}/UKG_Test_Attend`),
            fetch(`${API_BASE}/${SHEET_ID}/UKG_Test_Marks`)
        ]);
...
async function fetchResourcesData() {
    try {
        ...
        const data = await response.json();
        resourcesData = data.filter(row => isClassUKG(row['Select Class']));
    } catch (error) {
...
async function fetchMathsMarksData() {
    try {
        ...
        const data = await response.json();
        mathsMarksData = data.filter(row => isClassUKG(row.Class));
    } catch (error) {
...
async function fetchNotebookData() {
    try {
        ...
        const data = await response.json();
        notebookData = data.filter(row => isClassUKG(row.Class));
    } catch (error) {
...
// PPT Materials
function showPPT() {
    popupTitle.textContent = "PPT Materials - Class UKG";
        ...
        if (classUKGPPT.length === 0) {
            document.getElementById('pptContainer-materials').innerHTML = `
                <div style="text-align: center; padding: 40px; color: var(--muted);">
                    ...
                    <p>No lecture materials found for Class UKG.</p>
                </div>
            `;
            return;
        }
        
        displayPPTMaterials(classUKGPPT);
    } catch (error) {
...
function showAllStudentsMarks() {
    ...
    testMarksData.forEach(test => {
        ...
        studentNamesUKG.forEach(studentName => {
    ...
    const sortedStudents = studentNamesUKG.sort((a, b) => a.localeCompare(b));
...
function downloadStudentMarksReport(studentName) {
    ...
    doc.setFontSize(12);
    doc.text("Class UKG - Academic Year 2025-26", 105, 40, { align: "center" });
    ...
    doc.text("Generated by Class UKG Dashboard", 105, 280, { align: "center" });
    ...
    html += `
        ...
        <div class="test-summary">
            <div class="summary-card">
                ...
                <div class="summary-value">${studentNamesUKG.length}</div>
            </div>
            <div class="summary-card">
...
async function loadQuickActions() {
    try {
        ...
        const classUKGActions = quickActionsData.filter(row => 
            isClassUKG(row.Class)
        );
        
        if (classUKGActions.length === 0) {
            container.innerHTML = '<p style="text-align: center; grid-column: 1 / -1; color: #666;">No quick actions available for Class UKG</p>';
            return;
        }
        ...
        classUKGActions.forEach(resource => {
...
function loadSMPSData() {
    fetch(SMPS_SHEET_URL)
        ...
        .then(data => {
            ...
            const classUKGData = studentData.filter(row => {
            ...
            if (classUKGData.length === 0) {
                document.getElementById('smpsPrintArea').innerHTML = `
                    <div style="text-align: center; padding: 40px; color: var(--light-muted);">
                        ...
                        <p>No SMPS data found for Class UKG with 'submit' status.</p>
                    </div>
                `;
                return;
            }

            smpsOriginalData = [smpsRawData[0], subHeaderRow, ...classUKGData];
...
function downloadSMPSExcel() {
    ...
    link.setAttribute("download", `SMPS_ClassUKG_Data_${new Date().toISOString().split('T')[0]}.csv`);
...
async function loadActivityData() {
    try {
        ...
        displayActivityData(classUKGActivities);
    } catch (error) {
...
function displayActivityData(activities) {
    ...
    if (activities.length === 0) {
        activityContent.innerHTML = '<p>No activities available for Class UKG.</p>';
        return;
    }
