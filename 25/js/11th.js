...
let studentNames11th = []; // Will be populated after loading
...
let class11thBirthdays = []; // Store only 11th class birthdays
...
function loadStudentData() {
    return new Promise((resolve, reject) => {
        ...
        if (window.STUDENT_DATA && window.STUDENT_DATA.STUDENT_DATA) {
            ...
            // Extract 11th class student names
            const classKeys = Object.keys(studentData);
            console.log('Available class keys:', classKeys);
            
            // Find 11th class key (handles various formats)
            const twelfthClassKey = classKeys.find(key => {
                const normalizedKey = key.toLowerCase().replace(/[^a-z0-9]/g, '');
                return normalizedKey === '11th' || normalizedKey === '11th' || normalizedKey.includes('11th');
            });
            
            if (twelfthClassKey) {
                console.log(`Found 11th class with key: "${twelfthClassKey}"`);
                studentNames11th = studentData[twelfthClassKey]
                    .filter(student => student && student.name)
                    .map(student => student.name.trim());
                console.log(`Loaded ${studentNames11th.length} students for Class 11th:`, studentNames11th);
            } else {
                console.error('11th class data not found in STUDENT_DATA');
                // Try any key that might be 11th
                for (const key of classKeys) {
                    if (key.toLowerCase().includes('11th')) {
                        studentNames11th = studentData[key]
                            .filter(student => student && student.name)
                            .map(student => student.name.trim());
                        console.log(`Loaded ${studentNames11th.length} students using key: "${key}"`);
                        break;
                    }
                }
            }
            
            if (studentNames11th.length === 0) {
                console.warn('No student names found for Class 11th');
            }
            ...
            fetch('json/class.json')
                ...
                .then(data => {
                    ...
                    if (twelfthClassKey) {
                        ...
                        studentNames11th = studentData[twelfthClassKey]
                            .filter(student => student && student.name)
                            .map(student => student.name.trim());
                        console.log(`Loaded ${studentNames11th.length} students for Class 11th`);
                    } else {
                        console.error('11th class data not found in STUDENT_DATA');
                        // Fallback
                        const fallbackKey = classKeys.find(key => key.toLowerCase().includes('11th'));
                        if (fallbackKey) {
                            studentNames11th = studentData[fallbackKey]
                                .filter(student => student && student.name)
                                .map(student => student.name.trim());
                            console.log(`Loaded ${studentNames11th.length} students using fallback key: "${fallbackKey}"`);
                        }
                    }
                ...
                .catch(error => {
                    ...
                    studentNames11th = []; // Empty array as fallback
                    reject(error);
                });
        }
    });
...
// Helper function to check if class is 11th
function isClass11th(className) {
    if (!className) return false;
    const normalized = normalizeClassName(className);
    return normalized === '11th' || normalized === '11th' || normalized.includes('11th');
}
...
async function loadBirthdays() {
    try {
        ...
        // Filter for 11th class only using the helper function
        class11thBirthdays = data.filter(row => {
            if (!row || !row.class) return false;
            return isClass11th(row.class);
        });
        
        console.log(`Class 11th birthdays found: ${class11thBirthdays.length}`);
        ...
        class11thBirthdays.forEach(row => {
            if (!row.dob) return;
        ...
        if (todayList.length > 0) {
            ...
            todayList.forEach(birthday => {
                ...
                html += `
                    <div class="birthday-card class-11th">
                        ...
                        <div class="student-class">Class 11th</div>
...
function showCurrentMonthBirthdays() {
    ...
    const monthBirthdays = class11thBirthdays.filter(row => {
    ...
    if (monthBirthdays.length > 0) {
        monthBirthdays.forEach(birthday => {
            ...
            birthdaysHTML += `
                <div class="student-item class-11th">
...
// Filter notices by month AND Class (11th ya All)
function filterNotices(notices, month) {
    ...
    // Then filter by Class column (show only if Class is 'All' or 11th)
    filtered = filtered.filter(notice => {
        const noticeClass = notice.Class ? notice.Class.toString().trim() : '';
        return noticeClass.toLowerCase() === 'all' || isClass11th(noticeClass);
    });
...
function updateStats(notices) {
    ...
    notices.forEach(notice => {
        ...
        // Count notices for Class 11th
        const noticeClass = notice.Class ? notice.Class.toString().trim() : '';
        if (noticeClass.toLowerCase() === 'all' || isClass11th(noticeClass)) {
            class11thCount++;
        }
    });
...
async function fetchNoticeData() {
    try {
        ...
        const class11thNotices = data.filter(row => {
            ...
            return isClass11th(rowClass) && rowDateFormatted === todayFormatted && row.Notices;
        });
        
        if (class11thNotices.length > 0) {
            const latestNotice = class11thNotices[0];
    ...
    try {
        ...
        if (todaysHouseAssembly) {
            ...
            if (todaysClassAssembly) {
                ...
                if (isClass11th(todayAssembly.className)) {
                    applyThemeBasedOnHouse('11th');
                    showAssemblyBanner('Class 11th');
...
function showAssemblyBanner(houseOrClassName) {
    ...
    const bannerColors = {
        ...
        '11th': 'linear-gradient(135deg, #1565c0, #1976d2, #2196f3)'
    };
...
async function checkActivityData() {
    try {
        ...
        const class11thActivities = data.filter(row => 
            isClass11th(row.Class) || isClass11th(row['Select Class'])
        );
        
        if (class11thActivities.length === 0) {
...
async function checkStudyMaterialData() {
    ...
    for (const material of studyMaterials) {
        try {
            ...
            let filteredData = [];
            filteredData = data.filter(row => isClass11th(row.Class));
...
async function checkPPTData() {
    try {
        ...
        const class11thPPT = data.filter(row => isClass11th(row.Class));
        
        if (class11thPPT.length === 0) {
...
async function fetchTeacherData() {
    try {
        ...
        const class11thTeacher = data.find(row => isClass11th(row.Class));
        if (class11thTeacher) {
            teacherData = {
                name: class11thTeacher["Teacher's name"],
                dob: class11thTeacher.dob
            };
...
async function fetchTestData() {
    try {
        const [attendanceRes, marksRes] = await Promise.all([
            fetch(`${API_BASE}/${SHEET_ID}/11th_Test_Attend`),
            fetch(`${API_BASE}/${SHEET_ID}/11th_Test_Marks`)
        ]);
...
async function fetchResourcesData() {
    try {
        ...
        const data = await response.json();
        resourcesData = data.filter(row => isClass11th(row['Select Class']));
    } catch (error) {
...
async function fetchMathsMarksData() {
    try {
        ...
        const data = await response.json();
        mathsMarksData = data.filter(row => isClass11th(row.Class));
    } catch (error) {
...
async function fetchNotebookData() {
    try {
        ...
        const data = await response.json();
        notebookData = data.filter(row => isClass11th(row.Class));
    } catch (error) {
...
// PPT Materials
function showPPT() {
    popupTitle.textContent = "PPT Materials - Class 11th";
        ...
        if (class11thPPT.length === 0) {
            document.getElementById('pptContainer-materials').innerHTML = `
                <div style="text-align: center; padding: 40px; color: var(--muted);">
                    ...
                    <p>No lecture materials found for Class 11th.</p>
                </div>
            `;
            return;
        }
        
        displayPPTMaterials(class11thPPT);
    } catch (error) {
...
function showAllStudentsMarks() {
    ...
    testMarksData.forEach(test => {
        ...
        studentNames11th.forEach(studentName => {
    ...
    const sortedStudents = studentNames11th.sort((a, b) => a.localeCompare(b));
...
function downloadStudentMarksReport(studentName) {
    ...
    doc.setFontSize(12);
    doc.text("Class 11th - Academic Year 2025-26", 105, 40, { align: "center" });
    ...
    doc.text("Generated by Class 11th Dashboard", 105, 280, { align: "center" });
    ...
    html += `
        ...
        <div class="test-summary">
            <div class="summary-card">
                ...
                <div class="summary-value">${studentNames11th.length}</div>
            </div>
            <div class="summary-card">
...
async function loadQuickActions() {
    try {
        ...
        const class11thActions = quickActionsData.filter(row => 
            isClass11th(row.Class)
        );
        
        if (class11thActions.length === 0) {
            container.innerHTML = '<p style="text-align: center; grid-column: 1 / -1; color: #666;">No quick actions available for Class 11th</p>';
            return;
        }
        ...
        class11thActions.forEach(resource => {
...
function loadSMPSData() {
    fetch(SMPS_SHEET_URL)
        ...
        .then(data => {
            ...
            const class11thData = studentData.filter(row => {
            ...
            if (class11thData.length === 0) {
                document.getElementById('smpsPrintArea').innerHTML = `
                    <div style="text-align: center; padding: 40px; color: var(--light-muted);">
                        ...
                        <p>No SMPS data found for Class 11th with 'submit' status.</p>
                    </div>
                `;
                return;
            }

            smpsOriginalData = [smpsRawData[0], subHeaderRow, ...class11thData];
...
function downloadSMPSExcel() {
    ...
    link.setAttribute("download", `SMPS_Class11th_Data_${new Date().toISOString().split('T')[0]}.csv`);
...
async function loadActivityData() {
    try {
        ...
        displayActivityData(class11thActivities);
    } catch (error) {
...
function displayActivityData(activities) {
    ...
    if (activities.length === 0) {
        activityContent.innerHTML = '<p>No activities available for Class 11th.</p>';
        return;
    }
