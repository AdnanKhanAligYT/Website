...
let studentNames2nd = []; // Will be populated after loading
...
let class2ndBirthdays = []; // Store only 2nd class birthdays
...
function loadStudentData() {
    return new Promise((resolve, reject) => {
        ...
        if (window.STUDENT_DATA && window.STUDENT_DATA.STUDENT_DATA) {
            ...
            // Extract 2nd class student names
            const classKeys = Object.keys(studentData);
            console.log('Available class keys:', classKeys);
            
            // Find 2nd class key (handles various formats)
            const twelfthClassKey = classKeys.find(key => {
                const normalizedKey = key.toLowerCase().replace(/[^a-z0-9]/g, '');
                return normalizedKey === '2nd' || normalizedKey === '2nd' || normalizedKey.includes('2nd');
            });
            
            if (twelfthClassKey) {
                console.log(`Found 2nd class with key: "${twelfthClassKey}"`);
                studentNames2nd = studentData[twelfthClassKey]
                    .filter(student => student && student.name)
                    .map(student => student.name.trim());
                console.log(`Loaded ${studentNames2nd.length} students for Class 2nd:`, studentNames2nd);
            } else {
                console.error('2nd class data not found in STUDENT_DATA');
                // Try any key that might be 2nd
                for (const key of classKeys) {
                    if (key.toLowerCase().includes('2nd')) {
                        studentNames2nd = studentData[key]
                            .filter(student => student && student.name)
                            .map(student => student.name.trim());
                        console.log(`Loaded ${studentNames2nd.length} students using key: "${key}"`);
                        break;
                    }
                }
            }
            
            if (studentNames2nd.length === 0) {
                console.warn('No student names found for Class 2nd');
            }
            ...
            fetch('json/class.json')
                ...
                .then(data => {
                    ...
                    if (twelfthClassKey) {
                        ...
                        studentNames2nd = studentData[twelfthClassKey]
                            .filter(student => student && student.name)
                            .map(student => student.name.trim());
                        console.log(`Loaded ${studentNames2nd.length} students for Class 2nd`);
                    } else {
                        console.error('2nd class data not found in STUDENT_DATA');
                        // Fallback
                        const fallbackKey = classKeys.find(key => key.toLowerCase().includes('2nd'));
                        if (fallbackKey) {
                            studentNames2nd = studentData[fallbackKey]
                                .filter(student => student && student.name)
                                .map(student => student.name.trim());
                            console.log(`Loaded ${studentNames2nd.length} students using fallback key: "${fallbackKey}"`);
                        }
                    }
                ...
                .catch(error => {
                    ...
                    studentNames2nd = []; // Empty array as fallback
                    reject(error);
                });
        }
    });
...
// Helper function to check if class is 2nd
function isClass2nd(className) {
    if (!className) return false;
    const normalized = normalizeClassName(className);
    return normalized === '2nd' || normalized === '2nd' || normalized.includes('2nd');
}
...
async function loadBirthdays() {
    try {
        ...
        // Filter for 2nd class only using the helper function
        class2ndBirthdays = data.filter(row => {
            if (!row || !row.class) return false;
            return isClass2nd(row.class);
        });
        
        console.log(`Class 2nd birthdays found: ${class2ndBirthdays.length}`);
        ...
        if (todayList.length > 0) {
            ...
            todayList.forEach(birthday => {
                ...
                html += `
                    <div class="birthday-card class-2nd">
                        ...
                        <div class="student-class">Class 2nd</div>
...
function showCurrentMonthBirthdays() {
    ...
    const monthBirthdays = class2ndBirthdays.filter(row => {
    ...
    if (monthBirthdays.length > 0) {
        monthBirthdays.forEach(birthday => {
            ...
            birthdaysHTML += `
                <div class="student-item class-2nd">
...
// Filter notices by month AND Class (2nd ya All)
function filterNotices(notices, month) {
    ...
    // Then filter by Class column (show only if Class is 'All' or 2nd)
    filtered = filtered.filter(notice => {
        const noticeClass = notice.Class ? notice.Class.toString().trim() : '';
        return noticeClass.toLowerCase() === 'all' || isClass2nd(noticeClass);
    });
...
function updateStats(notices) {
    ...
    notices.forEach(notice => {
        ...
        // Count notices for Class 2nd
        const noticeClass = notice.Class ? notice.Class.toString().trim() : '';
        if (noticeClass.toLowerCase() === 'all' || isClass2nd(noticeClass)) {
            class2ndCount++;
        }
    });
...
async function fetchNoticeData() {
    try {
        ...
        const class2ndNotices = data.filter(row => {
            ...
            return isClass2nd(rowClass) && rowDateFormatted === todayFormatted && row.Notices;
        });
        
        if (class2ndNotices.length > 0) {
            const latestNotice = class2ndNotices[0];
    ...
    try {
        ...
        if (todaysHouseAssembly) {
            ...
            if (todaysClassAssembly) {
                ...
                if (isClass2nd(todayAssembly.className)) {
                    applyThemeBasedOnHouse('2nd');
                    showAssemblyBanner('Class 2nd');
...
function showAssemblyBanner(houseOrClassName) {
    ...
    const bannerColors = {
        ...
        '2nd': 'linear-gradient(135deg, #1565c0, #1976d2, #2196f3)'
    };
...
async function checkActivityData() {
    try {
        ...
        const class2ndActivities = data.filter(row => 
            isClass2nd(row.Class) || isClass2nd(row['Select Class'])
        );
        
        if (class2ndActivities.length === 0) {
...
async function checkStudyMaterialData() {
    ...
    for (const material of studyMaterials) {
        try {
            ...
            let filteredData = [];
            filteredData = data.filter(row => isClass2nd(row.Class));
...
async function checkPPTData() {
    try {
        ...
        const class2ndPPT = data.filter(row => isClass2nd(row.Class));
        
        if (class2ndPPT.length === 0) {
...
async function fetchTeacherData() {
    try {
        ...
        const class2ndTeacher = data.find(row => isClass2nd(row.Class));
        if (class2ndTeacher) {
            teacherData = {
                name: class2ndTeacher["Teacher's name"],
                dob: class2ndTeacher.dob
            };
...
async function fetchTestData() {
    try {
        const [attendanceRes, marksRes] = await Promise.all([
            fetch(`${API_BASE}/${SHEET_ID}/2nd_Test_Attend`),
            fetch(`${API_BASE}/${SHEET_ID}/2nd_Test_Marks`)
        ]);
...
async function fetchResourcesData() {
    try {
        ...
        const data = await response.json();
        resourcesData = data.filter(row => isClass2nd(row['Select Class']));
    } catch (error) {
...
async function fetchMathsMarksData() {
    try {
        ...
        const data = await response.json();
        mathsMarksData = data.filter(row => isClass2nd(row.Class));
    } catch (error) {
...
async function fetchNotebookData() {
    try {
        ...
        const data = await response.json();
        notebookData = data.filter(row => isClass2nd(row.Class));
    } catch (error) {
...
// PPT Materials
function showPPT() {
    popupTitle.textContent = "PPT Materials - Class 2nd";
        ...
        if (class2ndPPT.length === 0) {
            document.getElementById('pptContainer-materials').innerHTML = `
                <div style="text-align: center; padding: 40px; color: var(--muted);">
                    ...
                    <p>No lecture materials found for Class 2nd.</p>
                </div>
            `;
            return;
        }
        
        displayPPTMaterials(class2ndPPT);
    } catch (error) {
...
function showAllStudentsMarks() {
    ...
    testMarksData.forEach(test => {
        ...
        studentNames2nd.forEach(studentName => {
    ...
    const sortedStudents = studentNames2nd.sort((a, b) => a.localeCompare(b));
...
function downloadStudentMarksReport(studentName) {
    ...
    doc.setFontSize(12);
    doc.text("Class 2nd - Academic Year 2025-26", 105, 40, { align: "center" });
    ...
    doc.text("Generated by Class 2nd Dashboard", 105, 280, { align: "center" });
    ...
    html += `
        ...
        <div class="test-summary">
            <div class="summary-card">
                ...
                <div class="summary-value">${studentNames2nd.length}</div>
            </div>
            <div class="summary-card">
...
async function loadQuickActions() {
    try {
        ...
        const class2ndActions = quickActionsData.filter(row => 
            isClass2nd(row.Class)
        );
        
        if (class2ndActions.length === 0) {
            container.innerHTML = '<p style="text-align: center; grid-column: 1 / -1; color: #666;">No quick actions available for Class 2nd</p>';
            return;
        }
        ...
        class2ndActions.forEach(resource => {
...
function loadSMPSData() {
    fetch(SMPS_SHEET_URL)
        ...
        .then(data => {
            ...
            const class2ndData = studentData.filter(row => {
            ...
            if (class2ndData.length === 0) {
                document.getElementById('smpsPrintArea').innerHTML = `
                    <div style="text-align: center; padding: 40px; color: var(--light-muted);">
                        ...
                        <p>No SMPS data found for Class 2nd with 'submit' status.</p>
                    </div>
                `;
                return;
            }

            smpsOriginalData = [smpsRawData[0], subHeaderRow, ...class2ndData];
...
function downloadSMPSExcel() {
    ...
    link.setAttribute("download", `SMPS_Class2nd_Data_${new Date().toISOString().split('T')[0]}.csv`);
...
async function loadActivityData() {
    try {
        ...
        displayActivityData(class2ndActivities);
    } catch (error) {
...
function displayActivityData(activities) {
    ...
    if (activities.length === 0) {
        activityContent.innerHTML = '<p>No activities available for Class 2nd.</p>';
        return;
    }
