...
let studentNames8th = []; // Will be populated after loading
...
let class8thBirthdays = []; // Store only 8th class birthdays
...
function loadStudentData() {
    return new Promise((resolve, reject) => {
        ...
        if (window.STUDENT_DATA && window.STUDENT_DATA.STUDENT_DATA) {
            ...
            // Extract 8th class student names
            const classKeys = Object.keys(studentData);
            console.log('Available class keys:', classKeys);
            
            // Find 8th class key (handles various formats)
            const twelfthClassKey = classKeys.find(key => {
                const normalizedKey = key.toLowerCase().replace(/[^a-z0-9]/g, '');
                return normalizedKey === '8th' || normalizedKey === '8th' || normalizedKey.includes('8th');
            });
            
            if (twelfthClassKey) {
                console.log(`Found 8th class with key: "${twelfthClassKey}"`);
                studentNames8th = studentData[twelfthClassKey]
                    .filter(student => student && student.name)
                    .map(student => student.name.trim());
                console.log(`Loaded ${studentNames8th.length} students for Class 8th:`, studentNames8th);
            } else {
                console.error('8th class data not found in STUDENT_DATA');
                // Try any key that might be 8th
                for (const key of classKeys) {
                    if (key.toLowerCase().includes('8th')) {
                        studentNames8th = studentData[key]
                            .filter(student => student && student.name)
                            .map(student => student.name.trim());
                        console.log(`Loaded ${studentNames8th.length} students using key: "${key}"`);
                        break;
                    }
                }
            }
            
            if (studentNames8th.length === 0) {
                console.warn('No student names found for Class 8th');
            }
            ...
            fetch('json/class.json')
                ...
                .then(data => {
                    ...
                    if (twelfthClassKey) {
                        ...
                        studentNames8th = studentData[twelfthClassKey]
                            .filter(student => student && student.name)
                            .map(student => student.name.trim());
                        console.log(`Loaded ${studentNames8th.length} students for Class 8th`);
                    } else {
                        console.error('8th class data not found in STUDENT_DATA');
                        // Fallback
                        const fallbackKey = classKeys.find(key => key.toLowerCase().includes('8th'));
                        if (fallbackKey) {
                            studentNames8th = studentData[fallbackKey]
                                .filter(student => student && student.name)
                                .map(student => student.name.trim());
                            console.log(`Loaded ${studentNames8th.length} students using fallback key: "${fallbackKey}"`);
                        }
                    }
                ...
                .catch(error => {
                    ...
                    studentNames8th = []; // Empty array as fallback
                    reject(error);
                });
        }
    });
...
// Helper function to check if class is 8th
function isClass8th(className) {
    if (!className) return false;
    const normalized = normalizeClassName(className);
    return normalized === '8th' || normalized === '8th' || normalized.includes('8th');
}
...
async function loadBirthdays() {
    try {
        ...
        // Filter for 8th class only using the helper function
        class8thBirthdays = data.filter(row => {
            if (!row || !row.class) return false;
            return isClass8th(row.class);
        });
        
        console.log(`Class 8th birthdays found: ${class8thBirthdays.length}`);
        ...
        if (todayList.length > 0) {
            ...
            todayList.forEach(birthday => {
                ...
                html += `
                    <div class="birthday-card class-8th">
                        ...
                        <div class="student-class">Class 8th</div>
...
function showCurrentMonthBirthdays() {
    ...
    const monthBirthdays = class8thBirthdays.filter(row => {
    ...
    if (monthBirthdays.length > 0) {
        monthBirthdays.forEach(birthday => {
            ...
            birthdaysHTML += `
                <div class="student-item class-8th">
...
// Filter notices by month AND Class (8th ya All)
function filterNotices(notices, month) {
    ...
    // Then filter by Class column (show only if Class is 'All' or 8th)
    filtered = filtered.filter(notice => {
        const noticeClass = notice.Class ? notice.Class.toString().trim() : '';
        return noticeClass.toLowerCase() === 'all' || isClass8th(noticeClass);
    });
...
function updateStats(notices) {
    ...
    notices.forEach(notice => {
        ...
        // Count notices for Class 8th
        const noticeClass = notice.Class ? notice.Class.toString().trim() : '';
        if (noticeClass.toLowerCase() === 'all' || isClass8th(noticeClass)) {
            class8thCount++;
        }
    });
...
async function fetchNoticeData() {
    try {
        ...
        const class8thNotices = data.filter(row => {
            ...
            return isClass8th(rowClass) && rowDateFormatted === todayFormatted && row.Notices;
        });
        
        if (class8thNotices.length > 0) {
            const latestNotice = class8thNotices[0];
    ...
    try {
        ...
        if (todaysHouseAssembly) {
            ...
            if (todaysClassAssembly) {
                ...
                if (isClass8th(todayAssembly.className)) {
                    applyThemeBasedOnHouse('8th');
                    showAssemblyBanner('Class 8th');
...
function showAssemblyBanner(houseOrClassName) {
    ...
    const bannerColors = {
        ...
        '8th': 'linear-gradient(135deg, #1565c0, #1976d2, #2196f3)'
    };
...
async function checkActivityData() {
    try {
        ...
        const class8thActivities = data.filter(row => 
            isClass8th(row.Class) || isClass8th(row['Select Class'])
        );
        
        if (class8thActivities.length === 0) {
...
async function checkStudyMaterialData() {
    ...
    for (const material of studyMaterials) {
        try {
            ...
            let filteredData = [];
            filteredData = data.filter(row => isClass8th(row.Class));
...
async function checkPPTData() {
    try {
        ...
        const class8thPPT = data.filter(row => isClass8th(row.Class));
        
        if (class8thPPT.length === 0) {
...
async function fetchTeacherData() {
    try {
        ...
        const class8thTeacher = data.find(row => isClass8th(row.Class));
        if (class8thTeacher) {
            teacherData = {
                name: class8thTeacher["Teacher's name"],
                dob: class8thTeacher.dob
            };
...
async function fetchTestData() {
    try {
        const [attendanceRes, marksRes] = await Promise.all([
            fetch(`${API_BASE}/${SHEET_ID}/8th_Test_Attend`),
            fetch(`${API_BASE}/${SHEET_ID}/8th_Test_Marks`)
        ]);
...
async function fetchResourcesData() {
    try {
        ...
        const data = await response.json();
        resourcesData = data.filter(row => isClass8th(row['Select Class']));
    } catch (error) {
...
async function fetchMathsMarksData() {
    try {
        ...
        const data = await response.json();
        mathsMarksData = data.filter(row => isClass8th(row.Class));
    } catch (error) {
...
async function fetchNotebookData() {
    try {
        ...
        const data = await response.json();
        notebookData = data.filter(row => isClass8th(row.Class));
    } catch (error) {
...
// PPT Materials
function showPPT() {
    popupTitle.textContent = "PPT Materials - Class 8th";
        ...
        if (class8thPPT.length === 0) {
            document.getElementById('pptContainer-materials').innerHTML = `
                <div style="text-align: center; padding: 40px; color: var(--muted);">
                    ...
                    <p>No lecture materials found for Class 8th.</p>
                </div>
            `;
            return;
        }
        
        displayPPTMaterials(class8thPPT);
    } catch (error) {
...
function showAllStudentsMarks() {
    ...
    testMarksData.forEach(test => {
        ...
        studentNames8th.forEach(studentName => {
    ...
    const sortedStudents = studentNames8th.sort((a, b) => a.localeCompare(b));
...
function downloadStudentMarksReport(studentName) {
    ...
    doc.setFontSize(12);
    doc.text("Class 8th - Academic Year 2025-26", 105, 40, { align: "center" });
    ...
    doc.text("Generated by Class 8th Dashboard", 105, 280, { align: "center" });
    ...
    html += `
        ...
        <div class="test-summary">
            <div class="summary-card">
                ...
                <div class="summary-value">${studentNames8th.length}</div>
            </div>
            <div class="summary-card">
...
async function loadQuickActions() {
    try {
        ...
        const class8thActions = quickActionsData.filter(row => 
            isClass8th(row.Class)
        );
        
        if (class8thActions.length === 0) {
            container.innerHTML = '<p style="text-align: center; grid-column: 1 / -1; color: #666;">No quick actions available for Class 8th</p>';
            return;
        }
        ...
        class8thActions.forEach(resource => {
...
function loadSMPSData() {
    fetch(SMPS_SHEET_URL)
        ...
        .then(data => {
            ...
            const class8thData = studentData.filter(row => {
            ...
            if (class8thData.length === 0) {
                document.getElementById('smpsPrintArea').innerHTML = `
                    <div style="text-align: center; padding: 40px; color: var(--light-muted);">
                        ...
                        <p>No SMPS data found for Class 8th with 'submit' status.</p>
                    </div>
                `;
                return;
            }

            smpsOriginalData = [smpsRawData[0], subHeaderRow, ...class8thData];
...
function downloadSMPSExcel() {
    ...
    link.setAttribute("download", `SMPS_Class8th_Data_${new Date().toISOString().split('T')[0]}.csv`);
...
async function loadActivityData() {
    try {
        ...
        displayActivityData(class8thActivities);
    } catch (error) {
...
function displayActivityData(activities) {
    ...
    if (activities.length === 0) {
        activityContent.innerHTML = '<p>No activities available for Class 8th.</p>';
        return;
    }
