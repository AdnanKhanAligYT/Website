...
let studentNamesLKG = []; // Will be populated after loading
...
let classLKGBirthdays = []; // Store only LKG class birthdays
...
function loadStudentData() {
    return new Promise((resolve, reject) => {
        ...
        if (window.STUDENT_DATA && window.STUDENT_DATA.STUDENT_DATA) {
            ...
            // Extract LKG class student names
            const classKeys = Object.keys(studentData);
            console.log('Available class keys:', classKeys);
            
            // Find LKG class key (handles various formats)
            const lkgClassKey = classKeys.find(key => {
                const normalizedKey = key.toLowerCase().replace(/[^a-z0-9]/g, '');
                return normalizedKey === 'lkg' || normalizedKey.includes('lkg');
            });
            
            if (lkgClassKey) {
                console.log(`Found LKG class with key: "${lkgClassKey}"`);
                studentNamesLKG = studentData[lkgClassKey]
                    .filter(student => student && student.name)
                    .map(student => student.name.trim());
                console.log(`Loaded ${studentNamesLKG.length} students for Class LKG:`, studentNamesLKG);
            } else {
                console.error('LKG class data not found in STUDENT_DATA');
                // Try any key that might be LKG
                for (const key of classKeys) {
                    if (key.toLowerCase().includes('lkg')) {
                        studentNamesLKG = studentData[key]
                            .filter(student => student && student.name)
                            .map(student => student.name.trim());
                        console.log(`Loaded ${studentNamesLKG.length} students using key: "${key}"`);
                        break;
                    }
                }
            }
            
            if (studentNamesLKG.length === 0) {
                console.warn('No student names found for Class LKG');
            }
            ...
            fetch('json/class.json')
                ...
                .then(data => {
                    ...
                    if (lkgClassKey) {
                        ...
                        studentNamesLKG = studentData[lkgClassKey]
                            .filter(student => student && student.name)
                            .map(student => student.name.trim());
                        console.log(`Loaded ${studentNamesLKG.length} students for Class LKG`);
                    } else {
                        console.error('LKG class data not found in STUDENT_DATA');
                        // Fallback
                        const fallbackKey = classKeys.find(key => key.toLowerCase().includes('lkg'));
                        if (fallbackKey) {
                            studentNamesLKG = studentData[fallbackKey]
                                .filter(student => student && student.name)
                                .map(student => student.name.trim());
                            console.log(`Loaded ${studentNamesLKG.length} students using fallback key: "${fallbackKey}"`);
                        }
                    }
                ...
                .catch(error => {
                    ...
                    studentNamesLKG = []; // Empty array as fallback
                    reject(error);
                });
        }
    });
...
// Helper function to check if class is LKG
function isClassLKG(className) {
    if (!className) return false;
    const normalized = normalizeClassName(className);
    return normalized === 'lkg' || normalized.includes('lkg');
}
...
async function loadBirthdays() {
    try {
        ...
        // Filter for LKG class only using the helper function
        classLKGBirthdays = data.filter(row => {
            if (!row || !row.class) return false;
            return isClassLKG(row.class);
        });
        
        console.log(`Class LKG birthdays found: ${classLKGBirthdays.length}`);
        ...
        if (todayList.length > 0) {
            ...
            todayList.forEach(birthday => {
                ...
                html += `
                    <div class="birthday-card class-LKG">
                        ...
                        <div class="student-class">Class LKG</div>
...
function showCurrentMonthBirthdays() {
    ...
    const monthBirthdays = classLKGBirthdays.filter(row => {
    ...
    if (monthBirthdays.length > 0) {
        monthBirthdays.forEach(birthday => {
            ...
            birthdaysHTML += `
                <div class="student-item class-LKG">
...
// Filter notices by month AND Class (LKG ya All)
function filterNotices(notices, month) {
    ...
    // Then filter by Class column (show only if Class is 'All' or LKG)
    filtered = filtered.filter(notice => {
        const noticeClass = notice.Class ? notice.Class.toString().trim() : '';
        return noticeClass.toLowerCase() === 'all' || isClassLKG(noticeClass);
    });
...
function updateStats(notices) {
    ...
    notices.forEach(notice => {
        ...
        // Count notices for Class LKG
        const noticeClass = notice.Class ? notice.Class.toString().trim() : '';
        if (noticeClass.toLowerCase() === 'all' || isClassLKG(noticeClass)) {
            classLKGCount++;
        }
    });
...
async function fetchNoticeData() {
    try {
        ...
        const classLKGNotices = data.filter(row => {
            ...
            return isClassLKG(rowClass) && rowDateFormatted === todayFormatted && row.Notices;
        });
        
        if (classLKGNotices.length > 0) {
            const latestNotice = classLKGNotices[0];
    ...
    try {
        ...
        if (todaysHouseAssembly) {
            ...
            if (todaysClassAssembly) {
                ...
                if (isClassLKG(todayAssembly.className)) {
                    applyThemeBasedOnHouse('LKG');
                    showAssemblyBanner('Class LKG');
...
function showAssemblyBanner(houseOrClassName) {
    ...
    const bannerColors = {
        ...
        'LKG': 'linear-gradient(135deg, #1565c0, #1976d2, #2196f3)'
    };
...
async function checkActivityData() {
    try {
        ...
        const classLKGActivities = data.filter(row => 
            isClassLKG(row.Class) || isClassLKG(row['Select Class'])
        );
        
        if (classLKGActivities.length === 0) {
...
async function checkStudyMaterialData() {
    ...
    for (const material of studyMaterials) {
        try {
            ...
            let filteredData = [];
            filteredData = data.filter(row => isClassLKG(row.Class));
...
async function checkPPTData() {
    try {
        ...
        const classLKGPPT = data.filter(row => isClassLKG(row.Class));
        
        if (classLKGPPT.length === 0) {
...
async function fetchTeacherData() {
    try {
        ...
        const classLKGTeacher = data.find(row => isClassLKG(row.Class));
        if (classLKGTeacher) {
            teacherData = {
                name: classLKGTeacher["Teacher's name"],
                dob: classLKGTeacher.dob
            };
...
async function fetchTestData() {
    try {
        const [attendanceRes, marksRes] = await Promise.all([
            fetch(`${API_BASE}/${SHEET_ID}/LKG_Test_Attend`),
            fetch(`${API_BASE}/${SHEET_ID}/LKG_Test_Marks`)
        ]);
...
async function fetchResourcesData() {
    try {
        ...
        const data = await response.json();
        resourcesData = data.filter(row => isClassLKG(row['Select Class']));
    } catch (error) {
...
async function fetchMathsMarksData() {
    try {
        ...
        const data = await response.json();
        mathsMarksData = data.filter(row => isClassLKG(row.Class));
    } catch (error) {
...
async function fetchNotebookData() {
    try {
        ...
        const data = await response.json();
        notebookData = data.filter(row => isClassLKG(row.Class));
    } catch (error) {
...
// PPT Materials
function showPPT() {
    popupTitle.textContent = "PPT Materials - Class LKG";
        ...
        if (classLKGPPT.length === 0) {
            document.getElementById('pptContainer-materials').innerHTML = `
                <div style="text-align: center; padding: 40px; color: var(--muted);">
                    ...
                    <p>No lecture materials found for Class LKG.</p>
                </div>
            `;
            return;
        }
        
        displayPPTMaterials(classLKGPPT);
    } catch (error) {
...
function showAllStudentsMarks() {
    ...
    testMarksData.forEach(test => {
        ...
        studentNamesLKG.forEach(studentName => {
    ...
    const sortedStudents = studentNamesLKG.sort((a, b) => a.localeCompare(b));
...
function downloadStudentMarksReport(studentName) {
    ...
    doc.setFontSize(12);
    doc.text("Class LKG - Academic Year 2025-26", 105, 40, { align: "center" });
    ...
    doc.text("Generated by Class LKG Dashboard", 105, 280, { align: "center" });
    ...
    html += `
        ...
        <div class="test-summary">
            <div class="summary-card">
                ...
                <div class="summary-value">${studentNamesLKG.length}</div>
            </div>
            <div class="summary-card">
...
async function loadQuickActions() {
    try {
        ...
        const classLKGActions = quickActionsData.filter(row => 
            isClassLKG(row.Class)
        );
        
        if (classLKGActions.length === 0) {
            container.innerHTML = '<p style="text-align: center; grid-column: 1 / -1; color: #666;">No quick actions available for Class LKG</p>';
            return;
        }
        ...
        classLKGActions.forEach(resource => {
...
function loadSMPSData() {
    fetch(SMPS_SHEET_URL)
        ...
        .then(data => {
            ...
            const classLKGData = studentData.filter(row => {
            ...
            if (classLKGData.length === 0) {
                document.getElementById('smpsPrintArea').innerHTML = `
                    <div style="text-align: center; padding: 40px; color: var(--light-muted);">
                        ...
                        <p>No SMPS data found for Class LKG with 'submit' status.</p>
                    </div>
                `;
                return;
            }

            smpsOriginalData = [smpsRawData[0], subHeaderRow, ...classLKGData];
...
function downloadSMPSExcel() {
    ...
    link.setAttribute("download", `SMPS_ClassLKG_Data_${new Date().toISOString().split('T')[0]}.csv`);
...
async function loadActivityData() {
    try {
        ...
        displayActivityData(classLKGActivities);
    } catch (error) {
...
function displayActivityData(activities) {
    ...
    if (activities.length === 0) {
        activityContent.innerHTML = '<p>No activities available for Class LKG.</p>';
        return;
    }
