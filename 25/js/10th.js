...
let studentNames10th = []; // Will be populated after loading
...
let class10thBirthdays = []; // Store only 10th class birthdays
...
function loadStudentData() {
    return new Promise((resolve, reject) => {
        ...
        if (window.STUDENT_DATA && window.STUDENT_DATA.STUDENT_DATA) {
            ...
            // Extract 10th class student names
            const classKeys = Object.keys(studentData);
            console.log('Available class keys:', classKeys);
            
            // Find 10th class key (handles various formats)
            const twelfthClassKey = classKeys.find(key => {
                const normalizedKey = key.toLowerCase().replace(/[^a-z0-9]/g, '');
                return normalizedKey === '10th' || normalizedKey === '10th' || normalizedKey.includes('10th');
            });
            
            if (twelfthClassKey) {
                console.log(`Found 10th class with key: "${twelfthClassKey}"`);
                studentNames10th = studentData[twelfthClassKey]
                    .filter(student => student && student.name)
                    .map(student => student.name.trim());
                console.log(`Loaded ${studentNames10th.length} students for Class 10th:`, studentNames10th);
            } else {
                console.error('10th class data not found in STUDENT_DATA');
                // Try any key that might be 10th
                for (const key of classKeys) {
                    if (key.toLowerCase().includes('10th')) {
                        studentNames10th = studentData[key]
                            .filter(student => student && student.name)
                            .map(student => student.name.trim());
                        console.log(`Loaded ${studentNames10th.length} students using key: "${key}"`);
                        break;
                    }
                }
            }
            
            if (studentNames10th.length === 0) {
                console.warn('No student names found for Class 10th');
            }
            ...
            fetch('json/class.json')
                ...
                .then(data => {
                    ...
                    if (twelfthClassKey) {
                        ...
                        studentNames10th = studentData[twelfthClassKey]
                            .filter(student => student && student.name)
                            .map(student => student.name.trim());
                        console.log(`Loaded ${studentNames10th.length} students for Class 10th`);
                    } else {
                        console.error('10th class data not found in STUDENT_DATA');
                        // Fallback
                        const fallbackKey = classKeys.find(key => key.toLowerCase().includes('10th'));
                        if (fallbackKey) {
                            studentNames10th = studentData[fallbackKey]
                                .filter(student => student && student.name)
                                .map(student => student.name.trim());
                            console.log(`Loaded ${studentNames10th.length} students using fallback key: "${fallbackKey}"`);
                        }
                    }
                ...
                .catch(error => {
                    ...
                    studentNames10th = []; // Empty array as fallback
                    reject(error);
                });
        }
    });
...
// Helper function to check if class is 10th
function isClass10th(className) {
    if (!className) return false;
    const normalized = normalizeClassName(className);
    return normalized === '10th' || normalized === '10th' || normalized.includes('10th');
}
...
async function loadBirthdays() {
    try {
        ...
        // Filter for 10th class only using the helper function
        class10thBirthdays = data.filter(row => {
            if (!row || !row.class) return false;
            return isClass10th(row.class);
        });
        
        console.log(`Class 10th birthdays found: ${class10thBirthdays.length}`);
        ...
        class10thBirthdays.forEach(row => {
            if (!row.dob) return;
        ...
        if (todayList.length > 0) {
            ...
            todayList.forEach(birthday => {
                ...
                html += `
                    <div class="birthday-card class-10th">
                        ...
                        <div class="student-class">Class 10th</div>
...
function showCurrentMonthBirthdays() {
    ...
    const monthBirthdays = class10thBirthdays.filter(row => {
    ...
    if (monthBirthdays.length > 0) {
        monthBirthdays.forEach(birthday => {
            ...
            birthdaysHTML += `
                <div class="student-item class-10th">
...
// Filter notices by month AND Class (10th ya All)
function filterNotices(notices, month) {
    ...
    // Then filter by Class column (show only if Class is 'All' or 10th)
    filtered = filtered.filter(notice => {
        const noticeClass = notice.Class ? notice.Class.toString().trim() : '';
        return noticeClass.toLowerCase() === 'all' || isClass10th(noticeClass);
    });
...
function updateStats(notices) {
    ...
    notices.forEach(notice => {
        ...
        // Count notices for Class 10th
        const noticeClass = notice.Class ? notice.Class.toString().trim() : '';
        if (noticeClass.toLowerCase() === 'all' || isClass10th(noticeClass)) {
            class10thCount++;
        }
    });
...
async function fetchNoticeData() {
    try {
        ...
        const class10thNotices = data.filter(row => {
            ...
            return isClass10th(rowClass) && rowDateFormatted === todayFormatted && row.Notices;
        });
        
        if (class10thNotices.length > 0) {
            const latestNotice = class10thNotices[0];
    ...
    try {
        ...
        if (todaysHouseAssembly) {
            ...
            if (todaysClassAssembly) {
                ...
                if (isClass10th(todayAssembly.className)) {
                    applyThemeBasedOnHouse('10th');
                    showAssemblyBanner('Class 10th');
...
function showAssemblyBanner(houseOrClassName) {
    ...
    const bannerColors = {
        ...
        '10th': 'linear-gradient(135deg, #1565c0, #1976d2, #2196f3)'
    };
...
async function checkActivityData() {
    try {
        ...
        const class10thActivities = data.filter(row => 
            isClass10th(row.Class) || isClass10th(row['Select Class'])
        );
        
        if (class10thActivities.length === 0) {
...
async function checkStudyMaterialData() {
    ...
    for (const material of studyMaterials) {
        try {
            ...
            let filteredData = [];
            filteredData = data.filter(row => isClass10th(row.Class));
...
async function checkPPTData() {
    try {
        ...
        const class10thPPT = data.filter(row => isClass10th(row.Class));
        
        if (class10thPPT.length === 0) {
...
async function fetchTeacherData() {
    try {
        ...
        const class10thTeacher = data.find(row => isClass10th(row.Class));
        if (class10thTeacher) {
            teacherData = {
                name: class10thTeacher["Teacher's name"],
                dob: class10thTeacher.dob
            };
...
async function fetchTestData() {
    try {
        const [attendanceRes, marksRes] = await Promise.all([
            fetch(`${API_BASE}/${SHEET_ID}/10th_Test_Attend`),
            fetch(`${API_BASE}/${SHEET_ID}/10th_Test_Marks`)
        ]);
...
async function fetchResourcesData() {
    try {
        ...
        const data = await response.json();
        resourcesData = data.filter(row => isClass10th(row['Select Class']));
    } catch (error) {
...
async function fetchMathsMarksData() {
    try {
        ...
        const data = await response.json();
        mathsMarksData = data.filter(row => isClass10th(row.Class));
    } catch (error) {
...
async function fetchNotebookData() {
    try {
        ...
        const data = await response.json();
        notebookData = data.filter(row => isClass10th(row.Class));
    } catch (error) {
...
// PPT Materials
function showPPT() {
    popupTitle.textContent = "PPT Materials - Class 10th";
        ...
        if (class10thPPT.length === 0) {
            document.getElementById('pptContainer-materials').innerHTML = `
                <div style="text-align: center; padding: 40px; color: var(--muted);">
                    ...
                    <p>No lecture materials found for Class 10th.</p>
                </div>
            `;
            return;
        }
        
        displayPPTMaterials(class10thPPT);
    } catch (error) {
...
function showAllStudentsMarks() {
    ...
    testMarksData.forEach(test => {
        ...
        studentNames10th.forEach(studentName => {
    ...
    const sortedStudents = studentNames10th.sort((a, b) => a.localeCompare(b));
...
function downloadStudentMarksReport(studentName) {
    ...
    doc.setFontSize(12);
    doc.text("Class 10th - Academic Year 2025-26", 105, 40, { align: "center" });
    ...
    doc.text("Generated by Class 10th Dashboard", 105, 280, { align: "center" });
    ...
    html += `
        ...
        <div class="test-summary">
            <div class="summary-card">
                ...
                <div class="summary-value">${studentNames10th.length}</div>
            </div>
            <div class="summary-card">
...
async function loadQuickActions() {
    try {
        ...
        const class10thActions = quickActionsData.filter(row => 
            isClass10th(row.Class)
        );
        
        if (class10thActions.length === 0) {
            container.innerHTML = '<p style="text-align: center; grid-column: 1 / -1; color: #666;">No quick actions available for Class 10th</p>';
            return;
        }
        ...
        class10thActions.forEach(resource => {
...
function loadSMPSData() {
    fetch(SMPS_SHEET_URL)
        ...
        .then(data => {
            ...
            const class10thData = studentData.filter(row => {
            ...
            if (class10thData.length === 0) {
                document.getElementById('smpsPrintArea').innerHTML = `
                    <div style="text-align: center; padding: 40px; color: var(--light-muted);">
                        ...
                        <p>No SMPS data found for Class 10th with 'submit' status.</p>
                    </div>
                `;
                return;
            }

            smpsOriginalData = [smpsRawData[0], subHeaderRow, ...class10thData];
...
function downloadSMPSExcel() {
    ...
    link.setAttribute("download", `SMPS_Class10th_Data_${new Date().toISOString().split('T')[0]}.csv`);
...
async function loadActivityData() {
    try {
        ...
        displayActivityData(class10thActivities);
    } catch (error) {
...
function displayActivityData(activities) {
    ...
    if (activities.length === 0) {
        activityContent.innerHTML = '<p>No activities available for Class 10th.</p>';
        return;
    }
