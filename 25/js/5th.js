...
let studentNames5th = []; // Will be populated after loading
...
let class5thBirthdays = []; // Store only 5th class birthdays
...
function loadStudentData() {
    return new Promise((resolve, reject) => {
        ...
        if (window.STUDENT_DATA && window.STUDENT_DATA.STUDENT_DATA) {
            ...
            // Extract 5th class student names
            const classKeys = Object.keys(studentData);
            console.log('Available class keys:', classKeys);
            
            // Find 5th class key (handles various formats)
            const twelfthClassKey = classKeys.find(key => {
                const normalizedKey = key.toLowerCase().replace(/[^a-z0-9]/g, '');
                return normalizedKey === '5th' || normalizedKey === '5th' || normalizedKey.includes('5th');
            });
            
            if (twelfthClassKey) {
                console.log(`Found 5th class with key: "${twelfthClassKey}"`);
                studentNames5th = studentData[twelfthClassKey]
                    .filter(student => student && student.name)
                    .map(student => student.name.trim());
                console.log(`Loaded ${studentNames5th.length} students for Class 5th:`, studentNames5th);
            } else {
                console.error('5th class data not found in STUDENT_DATA');
                // Try any key that might be 5th
                for (const key of classKeys) {
                    if (key.toLowerCase().includes('5th')) {
                        studentNames5th = studentData[key]
                            .filter(student => student && student.name)
                            .map(student => student.name.trim());
                        console.log(`Loaded ${studentNames5th.length} students using key: "${key}"`);
                        break;
                    }
                }
            }
            
            if (studentNames5th.length === 0) {
                console.warn('No student names found for Class 5th');
            }
            ...
            fetch('json/class.json')
                ...
                .then(data => {
                    ...
                    if (twelfthClassKey) {
                        ...
                        studentNames5th = studentData[twelfthClassKey]
                            .filter(student => student && student.name)
                            .map(student => student.name.trim());
                        console.log(`Loaded ${studentNames5th.length} students for Class 5th`);
                    } else {
                        console.error('5th class data not found in STUDENT_DATA');
                        // Fallback
                        const fallbackKey = classKeys.find(key => key.toLowerCase().includes('5th'));
                        if (fallbackKey) {
                            studentNames5th = studentData[fallbackKey]
                                .filter(student => student && student.name)
                                .map(student => student.name.trim());
                            console.log(`Loaded ${studentNames5th.length} students using fallback key: "${fallbackKey}"`);
                        }
                    }
                ...
                .catch(error => {
                    ...
                    studentNames5th = []; // Empty array as fallback
                    reject(error);
                });
        }
    });
...
// Helper function to check if class is 5th
function isClass5th(className) {
    if (!className) return false;
    const normalized = normalizeClassName(className);
    return normalized === '5th' || normalized === '5th' || normalized.includes('5th');
}
...
async function loadBirthdays() {
    try {
        ...
        // Filter for 5th class only using the helper function
        class5thBirthdays = data.filter(row => {
            if (!row || !row.class) return false;
            return isClass5th(row.class);
        });
        
        console.log(`Class 5th birthdays found: ${class5thBirthdays.length}`);
        ...
        if (todayList.length > 0) {
            ...
            todayList.forEach(birthday => {
                ...
                html += `
                    <div class="birthday-card class-5th">
                        ...
                        <div class="student-class">Class 5th</div>
...
function showCurrentMonthBirthdays() {
    ...
    const monthBirthdays = class5thBirthdays.filter(row => {
    ...
    if (monthBirthdays.length > 0) {
        monthBirthdays.forEach(birthday => {
            ...
            birthdaysHTML += `
                <div class="student-item class-5th">
...
// Filter notices by month AND Class (5th ya All)
function filterNotices(notices, month) {
    ...
    // Then filter by Class column (show only if Class is 'All' or 5th)
    filtered = filtered.filter(notice => {
        const noticeClass = notice.Class ? notice.Class.toString().trim() : '';
        return noticeClass.toLowerCase() === 'all' || isClass5th(noticeClass);
    });
...
function updateStats(notices) {
    ...
    notices.forEach(notice => {
        ...
        // Count notices for Class 5th
        const noticeClass = notice.Class ? notice.Class.toString().trim() : '';
        if (noticeClass.toLowerCase() === 'all' || isClass5th(noticeClass)) {
            class5thCount++;
        }
    });
...
async function fetchNoticeData() {
    try {
        ...
        const class5thNotices = data.filter(row => {
            ...
            return isClass5th(rowClass) && rowDateFormatted === todayFormatted && row.Notices;
        });
        
        if (class5thNotices.length > 0) {
            const latestNotice = class5thNotices[0];
    ...
    try {
        ...
        if (todaysHouseAssembly) {
            ...
            if (todaysClassAssembly) {
                ...
                if (isClass5th(todayAssembly.className)) {
                    applyThemeBasedOnHouse('5th');
                    showAssemblyBanner('Class 5th');
...
function showAssemblyBanner(houseOrClassName) {
    ...
    const bannerColors = {
        ...
        '5th': 'linear-gradient(135deg, #1565c0, #1976d2, #2196f3)'
    };
...
async function checkActivityData() {
    try {
        ...
        const class5thActivities = data.filter(row => 
            isClass5th(row.Class) || isClass5th(row['Select Class'])
        );
        
        if (class5thActivities.length === 0) {
...
async function checkStudyMaterialData() {
    ...
    for (const material of studyMaterials) {
        try {
            ...
            let filteredData = [];
            filteredData = data.filter(row => isClass5th(row.Class));
...
async function checkPPTData() {
    try {
        ...
        const class5thPPT = data.filter(row => isClass5th(row.Class));
        
        if (class5thPPT.length === 0) {
...
async function fetchTeacherData() {
    try {
        ...
        const class5thTeacher = data.find(row => isClass5th(row.Class));
        if (class5thTeacher) {
            teacherData = {
                name: class5thTeacher["Teacher's name"],
                dob: class5thTeacher.dob
            };
...
async function fetchTestData() {
    try {
        const [attendanceRes, marksRes] = await Promise.all([
            fetch(`${API_BASE}/${SHEET_ID}/5th_Test_Attend`),
            fetch(`${API_BASE}/${SHEET_ID}/5th_Test_Marks`)
        ]);
...
async function fetchResourcesData() {
    try {
        ...
        const data = await response.json();
        resourcesData = data.filter(row => isClass5th(row['Select Class']));
    } catch (error) {
...
async function fetchMathsMarksData() {
    try {
        ...
        const data = await response.json();
        mathsMarksData = data.filter(row => isClass5th(row.Class));
    } catch (error) {
...
async function fetchNotebookData() {
    try {
        ...
        const data = await response.json();
        notebookData = data.filter(row => isClass5th(row.Class));
    } catch (error) {
...
// PPT Materials
function showPPT() {
    popupTitle.textContent = "PPT Materials - Class 5th";
        ...
        if (class5thPPT.length === 0) {
            document.getElementById('pptContainer-materials').innerHTML = `
                <div style="text-align: center; padding: 40px; color: var(--muted);">
                    ...
                    <p>No lecture materials found for Class 5th.</p>
                </div>
            `;
            return;
        }
        
        displayPPTMaterials(class5thPPT);
    } catch (error) {
...
function showAllStudentsMarks() {
    ...
    testMarksData.forEach(test => {
        ...
        studentNames5th.forEach(studentName => {
    ...
    const sortedStudents = studentNames5th.sort((a, b) => a.localeCompare(b));
...
function downloadStudentMarksReport(studentName) {
    ...
    doc.setFontSize(12);
    doc.text("Class 5th - Academic Year 2025-26", 105, 40, { align: "center" });
    ...
    doc.text("Generated by Class 5th Dashboard", 105, 280, { align: "center" });
    ...
    html += `
        ...
        <div class="test-summary">
            <div class="summary-card">
                ...
                <div class="summary-value">${studentNames5th.length}</div>
            </div>
            <div class="summary-card">
...
async function loadQuickActions() {
    try {
        ...
        const class5thActions = quickActionsData.filter(row => 
            isClass5th(row.Class)
        );
        
        if (class5thActions.length === 0) {
            container.innerHTML = '<p style="text-align: center; grid-column: 1 / -1; color: #666;">No quick actions available for Class 5th</p>';
            return;
        }
        ...
        class5thActions.forEach(resource => {
...
function loadSMPSData() {
    fetch(SMPS_SHEET_URL)
        ...
        .then(data => {
            ...
            const class5thData = studentData.filter(row => {
            ...
            if (class5thData.length === 0) {
                document.getElementById('smpsPrintArea').innerHTML = `
                    <div style="text-align: center; padding: 40px; color: var(--light-muted);">
                        ...
                        <p>No SMPS data found for Class 5th with 'submit' status.</p>
                    </div>
                `;
                return;
            }

            smpsOriginalData = [smpsRawData[0], subHeaderRow, ...class5thData];
...
function downloadSMPSExcel() {
    ...
    link.setAttribute("download", `SMPS_Class5th_Data_${new Date().toISOString().split('T')[0]}.csv`);
...
async function loadActivityData() {
    try {
        ...
        displayActivityData(class5thActivities);
    } catch (error) {
...
function displayActivityData(activities) {
    ...
    if (activities.length === 0) {
        activityContent.innerHTML = '<p>No activities available for Class 5th.</p>';
        return;
    }
