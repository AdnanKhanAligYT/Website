...
let studentNames9th = []; // Will be populated after loading
...
let class9thBirthdays = []; // Store only 9th class birthdays
...
function loadStudentData() {
    return new Promise((resolve, reject) => {
        ...
        if (window.STUDENT_DATA && window.STUDENT_DATA.STUDENT_DATA) {
            ...
            // Extract 9th class student names
            const classKeys = Object.keys(studentData);
            console.log('Available class keys:', classKeys);
            
            // Find 9th class key (handles various formats)
            const twelfthClassKey = classKeys.find(key => {
                const normalizedKey = key.toLowerCase().replace(/[^a-z0-9]/g, '');
                return normalizedKey === '9th' || normalizedKey === '9th' || normalizedKey.includes('9th');
            });
            
            if (twelfthClassKey) {
                console.log(`Found 9th class with key: "${twelfthClassKey}"`);
                studentNames9th = studentData[twelfthClassKey]
                    .filter(student => student && student.name)
                    .map(student => student.name.trim());
                console.log(`Loaded ${studentNames9th.length} students for Class 9th:`, studentNames9th);
            } else {
                console.error('9th class data not found in STUDENT_DATA');
                // Try any key that might be 9th
                for (const key of classKeys) {
                    if (key.toLowerCase().includes('9th')) {
                        studentNames9th = studentData[key]
                            .filter(student => student && student.name)
                            .map(student => student.name.trim());
                        console.log(`Loaded ${studentNames9th.length} students using key: "${key}"`);
                        break;
                    }
                }
            }
            
            if (studentNames9th.length === 0) {
                console.warn('No student names found for Class 9th');
            }
            ...
            fetch('json/class.json')
                ...
                .then(data => {
                    ...
                    if (twelfthClassKey) {
                        ...
                        studentNames9th = studentData[twelfthClassKey]
                            .filter(student => student && student.name)
                            .map(student => student.name.trim());
                        console.log(`Loaded ${studentNames9th.length} students for Class 9th`);
                    } else {
                        console.error('9th class data not found in STUDENT_DATA');
                        // Fallback
                        const fallbackKey = classKeys.find(key => key.toLowerCase().includes('9th'));
                        if (fallbackKey) {
                            studentNames9th = studentData[fallbackKey]
                                .filter(student => student && student.name)
                                .map(student => student.name.trim());
                            console.log(`Loaded ${studentNames9th.length} students using fallback key: "${fallbackKey}"`);
                        }
                    }
                ...
                .catch(error => {
                    ...
                    studentNames9th = []; // Empty array as fallback
                    reject(error);
                });
        }
    });
...
// Helper function to check if class is 9th
function isClass9th(className) {
    if (!className) return false;
    const normalized = normalizeClassName(className);
    return normalized === '9th' || normalized === '9th' || normalized.includes('9th');
}
...
async function loadBirthdays() {
    try {
        ...
        // Filter for 9th class only using the helper function
        class9thBirthdays = data.filter(row => {
            if (!row || !row.class) return false;
            return isClass9th(row.class);
        });
        
        console.log(`Class 9th birthdays found: ${class9thBirthdays.length}`);
        ...
        if (todayList.length > 0) {
            ...
            todayList.forEach(birthday => {
                ...
                html += `
                    <div class="birthday-card class-9th">
                        ...
                        <div class="student-class">Class 9th</div>
...
function showCurrentMonthBirthdays() {
    ...
    const monthBirthdays = class9thBirthdays.filter(row => {
    ...
    if (monthBirthdays.length > 0) {
        monthBirthdays.forEach(birthday => {
            ...
            birthdaysHTML += `
                <div class="student-item class-9th">
...
// Filter notices by month AND Class (9th ya All)
function filterNotices(notices, month) {
    ...
    // Then filter by Class column (show only if Class is 'All' or 9th)
    filtered = filtered.filter(notice => {
        const noticeClass = notice.Class ? notice.Class.toString().trim() : '';
        return noticeClass.toLowerCase() === 'all' || isClass9th(noticeClass);
    });
...
function updateStats(notices) {
    ...
    notices.forEach(notice => {
        ...
        // Count notices for Class 9th
        const noticeClass = notice.Class ? notice.Class.toString().trim() : '';
        if (noticeClass.toLowerCase() === 'all' || isClass9th(noticeClass)) {
            class9thCount++;
        }
    });
...
async function fetchNoticeData() {
    try {
        ...
        const class9thNotices = data.filter(row => {
            ...
            return isClass9th(rowClass) && rowDateFormatted === todayFormatted && row.Notices;
        });
        
        if (class9thNotices.length > 0) {
            const latestNotice = class9thNotices[0];
    ...
    try {
        ...
        if (todaysHouseAssembly) {
            ...
            if (todaysClassAssembly) {
                ...
                if (isClass9th(todayAssembly.className)) {
                    applyThemeBasedOnHouse('9th');
                    showAssemblyBanner('Class 9th');
...
function showAssemblyBanner(houseOrClassName) {
    ...
    const bannerColors = {
        ...
        '9th': 'linear-gradient(135deg, #1565c0, #1976d2, #2196f3)'
    };
...
async function checkActivityData() {
    try {
        ...
        const class9thActivities = data.filter(row => 
            isClass9th(row.Class) || isClass9th(row['Select Class'])
        );
        
        if (class9thActivities.length === 0) {
...
async function checkStudyMaterialData() {
    ...
    for (const material of studyMaterials) {
        try {
            ...
            let filteredData = [];
            filteredData = data.filter(row => isClass9th(row.Class));
...
async function checkPPTData() {
    try {
        ...
        const class9thPPT = data.filter(row => isClass9th(row.Class));
        
        if (class9thPPT.length === 0) {
...
async function fetchTeacherData() {
    try {
        ...
        const class9thTeacher = data.find(row => isClass9th(row.Class));
        if (class9thTeacher) {
            teacherData = {
                name: class9thTeacher["Teacher's name"],
                dob: class9thTeacher.dob
            };
...
async function fetchTestData() {
    try {
        const [attendanceRes, marksRes] = await Promise.all([
            fetch(`${API_BASE}/${SHEET_ID}/9th_Test_Attend`),
            fetch(`${API_BASE}/${SHEET_ID}/9th_Test_Marks`)
        ]);
...
async function fetchResourcesData() {
    try {
        ...
        const data = await response.json();
        resourcesData = data.filter(row => isClass9th(row['Select Class']));
    } catch (error) {
...
async function fetchMathsMarksData() {
    try {
        ...
        const data = await response.json();
        mathsMarksData = data.filter(row => isClass9th(row.Class));
    } catch (error) {
...
async function fetchNotebookData() {
    try {
        ...
        const data = await response.json();
        notebookData = data.filter(row => isClass9th(row.Class));
    } catch (error) {
...
// PPT Materials
function showPPT() {
    popupTitle.textContent = "PPT Materials - Class 9th";
        ...
        if (class9thPPT.length === 0) {
            document.getElementById('pptContainer-materials').innerHTML = `
                <div style="text-align: center; padding: 40px; color: var(--muted);">
                    ...
                    <p>No lecture materials found for Class 9th.</p>
                </div>
            `;
            return;
        }
        
        displayPPTMaterials(class9thPPT);
    } catch (error) {
...
function showAllStudentsMarks() {
    ...
    testMarksData.forEach(test => {
        ...
        studentNames9th.forEach(studentName => {
    ...
    const sortedStudents = studentNames9th.sort((a, b) => a.localeCompare(b));
...
function downloadStudentMarksReport(studentName) {
    ...
    doc.setFontSize(12);
    doc.text("Class 9th - Academic Year 2025-26", 105, 40, { align: "center" });
    ...
    doc.text("Generated by Class 9th Dashboard", 105, 280, { align: "center" });
    ...
    html += `
        ...
        <div class="test-summary">
            <div class="summary-card">
                ...
                <div class="summary-value">${studentNames9th.length}</div>
            </div>
            <div class="summary-card">
...
async function loadQuickActions() {
    try {
        ...
        const class9thActions = quickActionsData.filter(row => 
            isClass9th(row.Class)
        );
        
        if (class9thActions.length === 0) {
            container.innerHTML = '<p style="text-align: center; grid-column: 1 / -1; color: #666;">No quick actions available for Class 9th</p>';
            return;
        }
        ...
        class9thActions.forEach(resource => {
...
function loadSMPSData() {
    fetch(SMPS_SHEET_URL)
        ...
        .then(data => {
            ...
            const class9thData = studentData.filter(row => {
            ...
            if (class9thData.length === 0) {
                document.getElementById('smpsPrintArea').innerHTML = `
                    <div style="text-align: center; padding: 40px; color: var(--light-muted);">
                        ...
                        <p>No SMPS data found for Class 9th with 'submit' status.</p>
                    </div>
                `;
                return;
            }

            smpsOriginalData = [smpsRawData[0], subHeaderRow, ...class9thData];
...
function downloadSMPSExcel() {
    ...
    link.setAttribute("download", `SMPS_Class9th_Data_${new Date().toISOString().split('T')[0]}.csv`);
...
async function loadActivityData() {
    try {
        ...
        displayActivityData(class9thActivities);
    } catch (error) {
...
function displayActivityData(activities) {
    ...
    if (activities.length === 0) {
        activityContent.innerHTML = '<p>No activities available for Class 9th.</p>';
        return;
    }
